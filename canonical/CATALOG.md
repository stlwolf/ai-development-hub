...
| output-format-rule | Conclusion → evidence → steps → risks → links output structure; related links required when the turn created or updated an Issue / PR / in-repo doc (§5: comments optional, bare `#N` insufficient, no padding, read-only turns exempt); URL form by destination (§6: bare URL to the terminal, `[label](url)` to Markdown renderers); plain-Japanese wording (§8: no decorative English, keep work-object names); operator-facing plain prose (§9: no telegram compression; chat + human-gate; reasoning / agent-channels / board exempt) | `rules/output-format-rule.md` |
