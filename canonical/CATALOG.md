...
| subagent-strategy-rule | Subagent delegation: custom agents first, one task per subagent, implementer-contract; delegation limits belong to the harness, not to the rule; routing gate (new thread escalation signal, PR-unit) | `rules/subagent-strategy-rule.md` |
