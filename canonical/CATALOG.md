...
| subagent-strategy-rule | Subagent delegation: custom agents first, one task per subagent, implementer-contract; concurrency / nesting limits live in harness env vars, not in the rule; routing gate (new thread escalation signal, PR-unit) | `rules/subagent-strategy-rule.md` |
