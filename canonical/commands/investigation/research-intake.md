...
キーワード検索は語彙の一致に依存するため、**構造が同型だが語彙が異なる Issue** を見落とす。前処理の最後に逆方向スキャンを実施する。

1. `gh issue list` で取得した全オープン Issue のタイトル・本文を走査する
2. 各 Issue の「問題構造」（何を解決しようとしているか）と、Step 2 で抽出したパターンの構造的同型性を確認する
3. 判断基準は語彙の一致ではなく「同じ構造の問題を別のドメインで扱っているか」

キーワード検索（順方向）で接続が見つからなかった Issue に対して重点的に実施する。
