...
- **`jq` は 1.4 以上が要る**（`wait` メッセージの切り詰めに文字列スライスを使うため）。ただし `-R -r` を `-Rr` へ縮めると短オプションの連結になり、下限が 1.5 へ上がる。**縮めないこと。** 根拠は #343 のゲート4で jq のソース（`jq-1.4` の `jv_aux.c` と `main.c`）まで当てて確定した。理由の詳細は `notify.sh` の該当箇所のコメントにある（#347）
...
**呼び方の誤りは `broken` の帯（1）から外してある。** `-h` / `--help` は 0、unknown option・余分な位置引数・`--days` の非整数は 2 である。

**ただし全部の判定帯から切り離したわけではない。** 上のとおり `2` はいまも `indeterminate` の帯であり、**呼び方の誤りはそこへ相乗りしている。** 承知のうえの妥協で、分離するかは #301（定期実行の配線）が exit を読むか JSON を読むかを決めてから判断する。
