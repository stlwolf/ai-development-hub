...
main() {
  ...
  if parse_rm "$cmd_clean"; then
    ...
    for _t in "${_rm_targets[@]}"; do
      ...
      case "$_t" in
        /*|.|..|'~'|'~/'*|'$HOME'|'$HOME/'*)
          deny "Destructive rm blocked: $cmd" "rm-destructive"
          ;;
      esac
    done
  ...
  if [[ "$cmd_clean" =~ ^chmod[[:space:]]+-R[[:space:]]+777[[:space:]]+/ ]]; then
    deny "Destructive chmod blocked: $cmd" "chmod-recursive-root"
  fi

  if [[ "$cmd_clean" =~ ^chown[[:space:]]+-R[[:space:]]+[^[:space:]]+[[:space:]]+/ ]]; then
    deny "Destructive chown blocked: $cmd" "chown-recursive-root"
  fi
  ...
  if [[ "$cmd_clean" =~ drop[[:space:]]+table ]] ||
    ...
    shopt -u nocasematch
    deny "Destructive SQL blocked: $cmd" "sql-destructive"
  fi
  shopt -u nocasematch

  if [[ "$cmd_clean" =~ ^mkfs ]]; then
    deny "Destructive mkfs blocked: $cmd" "mkfs"
  fi

  if [[ "$cmd_clean" =~ ^dd[[:space:]] ]] && [[ "$cmd_clean" =~ of=/dev/ ]]; then
    deny "Destructive dd blocked: $cmd" "dd-device"
  fi

  if [[ "$cmd_clean" =~ ^git[[:space:]]+reset[[:space:]].*--hard ]]; then
    deny "Destructive git reset blocked: $cmd" "git-reset-hard"
  fi
  ...
  if [[ "$cmd_clean" =~ ^git[[:space:]]+clean[[:space:]] ]]; then
    ...
    if [[ "$clean_args" == *f* ]] && [[ "$clean_args" == *d* ]] && [[ "$clean_args" == *x* ]]; then
      deny "Destructive git clean blocked: $cmd" "git-clean-fdx"
    fi
  fi
