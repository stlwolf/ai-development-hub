...
case "$mode" in
  wait)
    emoji="⌨️"
    # #343: cut -c は UTF-8 ロケールでのみ文字単位で、C / POSIX ではバイト単位に退化する。
    # 日本語（3バイト文字）が上限の途中で割れると不正な UTF-8 になり、それが OSC 777 の
    # エスケープ列と、fallback の terminal-notifier / osascript の argv へそのまま載る。
    # jq の文字列スライスはコードポイント単位でロケールを参照しないので、上限の意味
    # （80 文字）を保ったままロケール依存が消える。jq は冒頭の command -v jq で
    # 確認済みの既存依存なので、実行ファイル依存は増えない。
    # -R と -r を分けて書くのは意図的: 連結した -Rr は短オプションの連結を要求するので
    # jq 1.5 以降になるが、分けて書けば 1.4 まで下がる（スライス自体は 1.4 から動く）。
    # 他の jq 呼び出しと同じく失敗は握り潰す（古い jq ではここが空になるだけ）。
