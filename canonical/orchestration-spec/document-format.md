...
| `ref` | 必須 | string | durable な作業単位参照。**既定は issue 番号**（closure は PR 作成前なので PR 番号が未確定なことがある）。URL も可 | どの作業での観測か。揮発層を指すパスは不可（適用範囲は下記） |
| `state` | 必須 | string | 下の enum 7値 | 帰結 |
| `note` | 任意 | string | 1行（改行なし） | 短い補足。長い証拠は `ref` の先に置く。**書かないならキーを省く**（`note: null` や空値は検証で弾く） |

`ref` の hygiene の適用範囲（誤読を避けるため明示する）:

- **判定は path 形状の `ref` にだけ適用する**。scheme つきの URL（`https://…`）は対象外で、`#274` / `owner/repo#274` のような issue / PR 参照も path ではないので触れない。
- path 形状のものは、**正規化してから先頭一致で**拒否する（前後の空白と先頭の `./` を落として判定する）。拒否するのは揮発層（`.oe/` / `tmp/`）で**始まる**パス・絶対パス・`..` セグメントを含むパス。
- **途中一致では拒否しない**。`foo/tmp/bar` を含む自由文や、URL の中に `/tmp/` や `/.oe/` を含むものは正当な `ref` として通す（部分一致で弾くと自由文の `ref` を誤爆する）。
