...
committed 状態 store 層（git 管理・§2.5・#272）
    projects/orchestration-engine/docs/knowledge/<ULID>.md : negative knowledge の型付き item（ai-hub 実体・文書でなく状態 store・型定義 §3.4）
...
- 実体（ai-hub）: `projects/orchestration-engine/docs/knowledge/<ULID>.md`（engine 蒸留木の直下・1 item = 1 ファイル）。採用先向けの置き場規則・型定義・スキーマ・命名（ULID ファイル名）・検証はすべて §3.4。
...
**採用先（他リポジトリ）向けの置き場規則**: そのリポジトリの蒸留ドキュメント木のルート直下に `knowledge/` を置き（`decisions/` / `episodes/` / `plans/` の兄弟）、型付き item（ULID 名）をこのディレクトリに**直接**置く（`items/` サブディレクトリは作らない）。README も同ディレクトリに同居させる。エンジン独自のトップレベル名前空間は切らず、committed で存在する蒸留木を錨にする。**蒸留木が複数あるリポジトリでは store も木ごとに置く**（各木の `knowledge/`。段3 の列挙は各木の store を横断して見る）。既存の自由記述 knowledge ノートを持つ木とは混在させず、それらの無い木を store の実体に選ぶ。ai-hub での実体は engine 木の `projects/orchestration-engine/docs/knowledge/`（README も同所）。トップレベル `docs/knowledge/` の自由記述ノートは別物で不干渉。検証コマンド `validate-knowledge` の directory mode は ULID 名の item のみを対象にし、README 等の非 item ファイルは skip する。
...
- **negative knowledge（#62/#272）で収穫基準（非自明・再発しうる・行動を変える）を満たすもの → 型付き knowledge store（蒸留木ルート直下 `knowledge/`・§2.5/§3.4・ai-hub は `projects/orchestration-engine/docs/knowledge/`）へ収穫**。これが第3の昇格先。切り出しは episode closure 時に `episode-retrospective` の収穫 Step が担う（`validate-knowledge` を通して episode と同じブランチにコミット・保存 HG = owner マージ）。基準を満たさない negative knowledge は従来どおり episode 本文に残す
...
> 設計級コンテンツは closure / 掃除の前に discussion / decision へ昇格し、committed→working の参照は昇格先へ張り替える（grep で substance の実在を確認）。収穫基準を満たす negative knowledge は型付き knowledge store（蒸留木ルート直下 `knowledge/`・§3.4）へ収穫する（第3経路・§13.3）。
