...
|--------|------|---------|-------------|
| `verified` | 一次確認済み | The cited source directly supports the claim (entailment holds) | MUST be backed by an external source the author actually inspected. NEVER assigned on self-evaluation alone (§2) |
...
| `speculation` | 推測 | No supporting source | Mark explicitly (inline `Speculation:` prefix is acceptable) |
| `contradicted` | 要編集 | A source contradicts the claim | Flag for correction |

- Bind status at the **statement level** (per claim / sentence), not per paragraph.
- Each `verified` / `unverified-summary` claim carries its source as a URL or `file:line`.
...
- Selective verification (partial acceptance) is legitimate resource allocation, not negligence — but the unchecked portion stays `unverified-summary`, never silently promoted to `verified`.
