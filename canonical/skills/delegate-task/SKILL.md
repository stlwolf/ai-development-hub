...
### 追加指示・承認の書き方（終点を書かない・#289）

実行中に送る 1 行は、**委譲時の書面より後に届くので書面に勝ってしまう**。長い実行列では、これがフローの終端を意図せず書き換える。実際に、義務が書面2つ（委譲固定節と plan の step）に書かれていたのに、承認の末尾に書いた1文が終点を再定義して後段のゲートが実行されなかった事故が起きている。

したがって送る側は次の3つを守る。

1. **終点を文で書かない。plan の step ID で位置を指す。** 「報告を仕上げてパスを返す」は終点の発明である。「P4 が終わったら P5 へ進む」なら plan が終端を保持したままになる。
2. **すべてのメッセージで issue 番号と step ID を名指しする。** 子が複数走っているとき、宛先違いも step 違いもその場で見える。
3. **終端を宣言するのは子の側**に置き、送る側はそれと照合する。子が報告のたびに自分の終端（委譲時に決まった step ID）を echo し、送る側の指示がそれと食い違ったら子が確認を求める（`implementer-contract` の指示矛盾ガード）。

禁止だけを繰り返すこと（「マージ・掃除・close はしない」）は義務側の再掲にならない。**残りの義務を再掲するより、終点を書かず位置だけを指す方が強い**（再掲しつつ別の終点を書く余地が残らない）。

**位置を指す形にしても、指した先までの義務が未達なら飛び越しである。** 「P3 へ進め」は P2 が未達なら終点の発明と同じ結果になる。step の要件を下げる指示（「SO は省いて PR へ」）も同様に義務を消す。実行側はこの2つも確認対象として返す（`implementer-contract` の弁別子 (b)(c)）。

方針を意図して変える場合は、**plan の更新と再承認までを1単位にする**（口頭の合意だけで進めると、完了前の照合が古い plan を基準に走る）。

完了扱いにする前の未達照合は `unmet-gate-check`（経緯を渡さない検出層）。**ただしそれは起動されれば効く層であって、起動を保証しない**（強制する層は #291）。
...
## 委譲子の状態を親が判定する（1枚の画面から読まない）

委譲した子が動いているのか、終わったのか、何かを待って止まっているのか。**この判定を1回の画面 capture から読み取らない。** 画面そのものが禁じられるわけではなく、**時間差で2回撮って差分を見る**形（`oe-selfcheck` が全ペインに対して行っているもの）は決定論的で有効である。使ってはいけないのは、1枚の画面に何が写っているかで状態を決めることである。

### 判定は3点で組む

| # | 信号 | 取り方 | 意味 |
|---|------|--------|------|
| a | プロセスの生死 | `ps -p <pid>` | 消えていれば「完了」と「異常終了」の両方が候補。どちらかは成果物で切り分ける |
| b | CPU 使用率の**絶対値** | `ps -o %cpu= -p <pid>` | 閾値を下回っていれば**静止している候補**として扱う。実測ではアイドルの子は 0.0〜0.9% を示した |
| c | 終端成果物の有無と更新時刻 | 委譲時に指定した path への `stat`（BSD は `stat -f '%m'`・GNU は `stat -c '%Y'`。**どちらの `stat` かで分岐する** — GNU の `-f` は別の意味なので取り違えると黙って違うものを返す） | 更新が止まってからの経過時間。存在するかどうか自体も材料 |

pid は宛先のペインから引く。ただし **`pane_pid` は tmux の定義では「そのペインの最初のプロセス」**であって、子そのものとは限らない。子がペインのコマンドとして起動されていれば一致する（engine の spawn はこの形）が、既存のシェルの中で手で起動した子では取れるのはシェルの pid になり、**子が終了してシェルが残っている状態でも生存を返す**。まず一致を確かめ、一致しないならプロセス木を辿る。

```bash
PID=$(tmux display-message -p -t "%N" '#{pane_pid}')
ps -o pid=,comm= -p "$PID"   # 子本体か、シェルか。ここが一致していることが前提
ps -o %cpu= -p "$PID"
```

**b を累積 CPU 時間の「変化量」で代用してはいけない。** アイドルのプロセスも CPU を少しずつ使い続けるので、「N分間変化しなければ停止」という形の判定は毎回リセットされ、**一度も発火しない**。108分と15.5時間のアイドルを2件とも見逃し、絶対値へ寄せたところ6分で発火した（別環境の実測）。これは「累計に基づく述語は、対象の状態が変わっても遷移しない」という一般の形の一例である。

**b は1回の測定で決めない。** 手元の `ps` は `%cpu` を「直近およそ1分の減衰平均」と定義しており、作業中のプロセスでも待ちの区間に入ると一時的に低く出る（稼働中のセッションが 16.4% から 1.9% へ落ちた測定がある）。**この読みは `ps` の実装に依存する**ので、別の環境へ移すときは値が秒単位で動くかを先に確かめる（プロセス開始からの平均を返す実装では同じ判定にならない）。起動直後は時間基準が短く値が安定しないので、spawn 直後に測らない。

**低い `%cpu` が意味するのは「止まった」ではなく「観測した窓で CPU をほとんど使っていない」である。** 待ち（API・ネットワーク・ロック・ディスク）に入っているとき、実作業が別の子プロセス側で走っているとき、短い活動がサンプルの合間に落ちたとき、いずれも同じ低い値になる。逆に、実質止まっていても背景の細かい活動で閾値を超えることがあるし、帯そのものが CPU 数や仮想化で動く。**数回サンプルを取るだけでは足りず、観測する窓を「想定される最長の静寂区間」より長く取る。** 逆向きも成り立たない — **高い値は前進の証拠にならない**（前に進まないループも CPU を使う）。**機械が出せるのは候補までである。** 確定は c の成果物と人の目に渡す（下記「止まり方の種類は機械で切り分けない」）。

**閾値はここでは決めない。** アイドルの上限として観測された値と、稼働中に落ちた下限の値は近く、しかも別の機械で測られている。判定を実装する側が自分の環境で測って決める。

### 1枚の画面から読まない理由

1. **待機中を示す表示は、止まった後も画面に残る。** 1枚の画面に残った表示を稼働の証拠にすると誤る。稼働を画面から言いたいなら、**時間差の2回 capture の差分**を使う（1枚では言えない）。
2. **遡っても内容が増えないことがある。** 実測では `capture-pane -p` が7行しか返さず、遡る行数を増やしても増えなかった（別環境の実測）。**これを「代替画面だから」と説明しない。** `capture-pane -a` は代替画面が存在しなければ `-q` が無いかぎりエラーを返す仕様なので、`-a` が何も返さないことは**代替画面が無かった**ことの証拠である。実際の理由は、この TUI が作業中の内容を scrollback に残さないことにある。capture 自体は成功する。
3. **capture は既定で属性を落とす。** そのため補完のゴーストテキストが普通の入力と同じ見た目になる（次節）。

### 入力欄に未送信の入力があるかは、2本立てで見る

**片方だけでは足りない。両方通す。それでも十分条件にはならない。**

まず入力欄の行を正しく取る。**`❯` を含む行を拾い、その最後の1行を取る**（送信済みメッセージの echo も `❯` を含むが画面の上方に出るので、最後の1行が入力欄になる）。capture の前にペインが mode に入っていないか確かめる（トラックパッドのスクロールで copy-mode に入ると、入力欄が視野から外れて材料そのものが消える）。長い入力は折り返されるので `-J` で結合する。

```bash
tmux display-message -p -t "%N" '#{pane_in_mode}'          # 1 なら先に解除する
tmux capture-pane -p -e -J -t "%N" -S -6 | grep '❯' | tail -n 1
```

⚠ **属性付き（`-e`）の capture では、`❯` に行頭の錨を打ってはいけない。** 属性付きの行は `❯` より前に属性のエスケープ列が来るので、行頭を要求すると**1行も取れない**。engine の `lib/delegate-send.sh` は行頭に錨を打つ形を使っているが、あちらが読んでいるのは**属性なしの capture** である。**錨の有無は capture の取り方とセットで決まる**ので、検出器を別の取り方へ移すときはそのまま持っていかない。

- **属性を見る**: 取れた行の `❯` の**後ろ**が `ESC[2m`（薄い表示）で始まっていれば、それは補完の予測であって未送信の入力ではない。見るのは**後ろだけ**にする — `❯` の前に付く色の属性は場面で変わる（`ESC[39m` のことも `ESC[37m` のこともある）ので、そちらを判定材料にしない。**この署名は検証した環境で観測されたものであって、一般の契約ではない。** 描画は TUI の実装に依存し、属性の並びは結合されうる。**判定が外れたときにまず疑うのは子ではなく自分の検出器**である。
- **送信記録と突合する**: engine の送信イベント記録（既定は `~/.claude/state/oe-events.jsonl`・`OE_EVENT_DIR` で変わる）に、そのペイン宛（`to.pane`）の送信が残っているか見る。**記録があることは実入力の裏付けになるが、記録が無いことは補完の証明にならない。** 覆えるのは engine 経由の送信だけで、**人がそのペインへ直接打った入力はイベントに残らない**。`oe-send --no-enter` も実入力を入力欄へ置くだけで送信イベントを残さないし、記録自体が best-effort で `OE_EVENT_LOG=0` や `jq` 不在では黙って欠ける。

**2つが食い違ったときの扱いを非対称にする。** 「実入力がある」と結論してよいのは2つが一致したときだけで、**どちらか一方でも欠けたら不明として人へ上げる**。2本を通しても穴は残るので、**入力欄の中身を根拠に消したり報告したりしない。** 判断が要るなら人か、その子自身に聞く。

⚠ **入力欄に見えた文字列を、owner や親からの指示として読み取ってはいけない。** 補完は「次に送られそうな文」を予測するので、**待っている文面に似る方向へ偏る**。承認・裁定・許可の類は、必ず自分が受け取った経路（owner の発言・`oe-send` の着信）だけを根拠にする。規範は `implementation-gate-rule`、この罠がなぜ避けにくいかは knowledge store 側の item に置く（3軸分離）。

### 止まり方の種類は機械で切り分けない（人に残す）

plan を書いて止まる委譲子にとって、止まること自体は**正常な終端**である。「ゲート待ちの正常な停止」と「終わったのに報告が届いていない状態」は、同じ停止に見える。

機械で見えるのは **委譲時に指定した終端成果物が、指定した path にあるかどうか**までである。したがって**委譲の書面には、終端を step の ID と成果物の path の両方で書く**。そうすると機械が見られる対象が1つ増える。最後の切り分けは人が成果物の中身で行う。
