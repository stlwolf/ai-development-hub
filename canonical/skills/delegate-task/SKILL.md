...
まず入力欄の行を正しく取る。**行頭の `❯` に錨を打ち、最後の1行を取る**（送信済みメッセージの echo も `❯` で始まるので、行を絞らないと履歴行を入力欄と誤認する）。capture の前にペインが mode に入っていないか確かめる（トラックパッドのスクロールで copy-mode に入ると、入力欄が視野から外れて材料そのものが消える）。長い入力は折り返されるので `-J` で結合する。engine の実装（`lib/delegate-send.sh`）が同じ形を取っている。

```bash
tmux display-message -p -t "%N" '#{pane_in_mode}'          # 1 なら先に解除する
tmux capture-pane -p -e -J -t "%N" -S -6 | grep '^[[:space:]]*❯' | tail -n 1
```

- **属性を見る**: 取れた行の `❯` の**後ろ**が `ESC[2m`（薄い表示）で始まっていれば、それは補完の予測であって未送信の入力ではない。`❯` 自体は `ESC[39m`（既定色）である。**この署名は検証した環境で観測されたものであって、一般の契約ではない。** 描画は TUI の実装に依存し、属性の並びは結合されうる。**判定が外れたときにまず疑うのは子ではなく自分の検出器**である。
