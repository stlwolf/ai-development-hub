...
2. 以下の **4つの問い** を diff 全体に適用する。ヒントのスキャン方針: **Bash ヒント（`.sh` / `run:` / `` ```bash `` ブロックが diff にある場合）と汎用ヒント（任意の言語のコードが diff にある場合）は常時スキャン**。CDK/PHP/Python ヒントは **`--cdk`/`--php`/`--python` フラグ指定時かつ対象ファイルが diff に含まれる場合のみスキャン**（フラグなし = N/A）
