...
- **plan-first**: 実装に着手するのは owner HG の後。それまでは計画だけ作って STOP し、plan doc のパスを親へ報告する。**plan・episode・knowledge は作業の枝に載せ、実装の最初の PR と一緒に着地させる**（plan だけを単独の PR で default ブランチへ着地させない）。HG の前に plan や途中の実装を **draft PR** に載せて owner に pre-review してもらうのは可（draft のまま置き、マージしない）。文書だけの PR を着地させてよいのは、owner が明示的にそう裁定したときだけ。
