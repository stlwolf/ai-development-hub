...
**発火するのは2つ。** owner が「交代して」と言ったとき。もう1つは自分の context% が閾値に近いと気づいたときで、**この閾値は常駐の見張りが owner へ ping する値と同じ**である（既定 85%・`oe-vitals --threshold`。**見張りは `context_pct > T` を超えたときに ping し、宛先は owner なので統括には来ない**）。
...
1. **`oe-handoff prepare` を自分のセッションで走らせる。** 機械で取れる状態（open PR・worktree・未 push・生きた委譲子・見張り・自分の session_id と context% と pid）が引き継ぎ文書の**機械が書く節**へ入る。**この節は毎回まるごと上書きされるので手で直さない。** 置き場の既定はワークスペースの `.oe/handoff.md`（`--out` か `OE_HANDOFF_FILE` で動かせる）で、**会話にしか無かったものを書く場所**なので、**その置き場が版管理の対象外かを確かめる。** **exit 0 を確かめてから次へ進む** — `jq` が無い・目印が壊れている等で止まると、**古い機械の節のまま `start` してしまう。**
2. **人が書く節を4つとも埋める**（自己申告と残余 / owner が下した裁定と次のテーマ / 確立した運用 / 渡らないもの）。**前任にしか書けないのはここである。**
   ...
   - **申告と作業状態の照合で `retire` が止めるのは**、自己申告の節が無い / 表の行が0件 / 処分が語彙の外 / **機械の節が挙げた PR と未 push** / **実行時に在る open PR・未 push・main 以外の worktree**、が処分の付いた行に出てこないときである。**散文に書いても処分にはならない。** **open PR は作者で絞られない**ので、自分のでない PR にも処分が要る。**申告では直せない止まり方も別に在る**（`jq` や `gh` が無い・`git worktree list` が失敗する・board や pid の突合が合わない）。**止まる条件の正本は `$oe_bin/README.md`。**
