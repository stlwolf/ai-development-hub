...
closure は本文と同じ内容を2度書かない。**本文にある事実は、再掲でなく本文への pointer で足りる。**

- **本文にある事実を closure で再執筆しない。** 該当箇所を指すだけでよい。
...
**各項目は再掲でも pointer でもよい**（上記の read/write 契約）。本文に既に書いた事実は `` `本文: <見出し>` `` で指せば足りる。
...
- 既定: `so-compare` で振り返りの focused check。確認対象は **失敗セクションの選択的省略 / routing の網羅 / evidence anchor / back-propagation** のみに絞る（全文レビューにしない — 摩擦が高いと skip される）
  - **pointer は省略ではない**: 事実・失敗節の項目が `` `本文: <見出し>` `` で本文を指している場合、再掲が無いことを選択的省略と判定しない（read/write 契約）。省略かどうかは「**実行ログにある失敗・撤回・指摘が、事実・失敗節に項目として現れているか**」で見る。確認対象の範囲（closure 節）は変えない
