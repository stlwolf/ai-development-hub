...
- **本文にある事実には pointer を付ける。** 全文の再掲は不要で、**1行の要約を添えるかは任意**（要約だけを置いて pointer を省くことはしない）。
- **本文に無い事実を closure で新規作成しない。** 欠落を埋めた場合はその項目に補完である旨と理由を1行付ける（後述のマーカー）。
...
- **P1**: **Step 3 の内容セクションの各項目**（出力型を問わない）は、本文にある事実なら pointer を、本文に無い事実なら補完マーカーを持つ。
