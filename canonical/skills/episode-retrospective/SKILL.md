...
- **P1**: 事実・失敗節の各項目は、pointer か補完マーカーのどちらかを持つ。
...
主張の単位は**事実・失敗節の最上位の箇条書き1項目**とする（ネストは親に属する・非箇条の prose は対象外）。範囲を絞ることで判定を決定論にする。

**述語の役割を取り違えないこと。** これらは**省略を機械で塞ぐものではない**。省略の検出は Step 4（実行ログと事実・失敗節の突合）が担い続ける。述語が担うのは「二重執筆を止めつつ、詳細への導線を保つ」ことだけである。したがって**アンカー形式に過剰な精度を要らない** — 見出しの正規化や Markdown parser 級の厳密さは不要で、壊れても落ちるのは導線だけで省略検出は落ちない。
...
**本文に既に書いた事実は `` `本文: <見出し>` `` を付ける**（上記の read/write 契約）。全文の再掲は要らず、1行の要約を添えるかは任意である。本文に無い事実には補完マーカーを付ける。
