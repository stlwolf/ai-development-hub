...
- [ ] **昇格の判定**: この単位が下した判断を挙げ、**1件ずつ**判定（`required` / `not-required`（理由）/ `unknown`）を書いたか。**集計の「昇格なし」1行では閉じられない。** **扱える範囲・判定の基準・`unknown` の使い分けは下記「昇格の判定」節（範囲は節の冒頭にある。扱うのは判定までで実行は本スキルの外）。** **「該当なし」は正当な結果**（件数は運用指標にしない）。各判定は、**本節の read/write 契約と同じ pointer 形式**（`` `本文: <見出し>` ``）で**本文の根拠箇所を指す**。判定に対応する候補型は Step 3 の該当行（蒸留シグナル / 原則）に書き、そちらも同じ pointer 形式に従う（述語 P1 の対象になるため）
...
上の必須項目「昇格の判定」を**どこに書くか**と、**作業中の印をどう扱うか**。判定の基準そのものは下記「昇格の判定」節が持ち、本項はそれを変えない。

**判定は二重アンカーで拾う。** 一次アンカーは**作業中にその場で置く印**（`昇格の印:` で始まる本文の1行。規約は `document-format.md`「ライフサイクル規範」節）。最後のアンカーが**上の必須項目**である。

- **closure は本文の印を拾う。** 印の集合を判定の分母の候補として使う。**各印を明示的に検討し、同じ判断に複数の印が付いている場合をまとめるほかは、印1件につき `promotion` エントリを1件置く。** closure 側の判定項目と `promotion.subject` には同じ1行要約を使い、突き合わせられるようにする。
- **かつ、印が1つも無くても独立に問う。** 理想はゲートが1個の常に動くコンポーネントとして毎回判断してくれることだが、そうはならない前提で、**それが動かなかった場合に最後にもう一度確認する**のが closure の役目である。したがって**本項目を「印を拾うだけ」に退化させてはいけない。** 印の無い判断を closure で見つけたら、それも判定の対象にする。
- closure 自身の役目（作業を終えたと思った後から、マージされて承認が出るまでに起きたことの振り返り）は**変わらない**。

**判定の記録先**: episode の frontmatter `promotion` に**1判定 = 1エントリ**で置く（欄の定義・値域・多重度の正本は `document-format.md`「昇格の判定」節）。

- **判定の対象になる判断が1件も無かった場合だけ `promotion: []` と書く。** **`required` が0件だったことは `[]` では表さない** — `not-required` と `unknown` も1件ずつ記録する対象なので、N 件のエントリのどれも `required` でない形で表す。
- **上の必須項目が言う「該当なし」との対応**: 「該当なし」＝**判定の対象になる判断が0件**であり、これが `promotion: []` に当たる。**判断を挙げたうえで全部が `not-required` だったものは「該当なし」ではない** — その場合はエントリを N 件書く（畳むと記録が消える）。
- **判定の集計行は書かない**（件数が要るなら `promotion` から導出する）。**件数は運用指標にしない。**
- 理由・参照した材料・`unknown` の「何が分かれば決まるか」は**従来どおり closure 側に書く**（frontmatter へ移さない）。frontmatter が持つのは `subject` / `verdict` / `ref` の3つ組だけである。
- **フィールドの不在は未実施を意味しない**（opt-out tier と、episode を持たない単位があるため）。不在の解釈規則は同じく `document-format.md`「昇格の判定」節にある。

**正直に書く**: これは判定の基準の改訂ではないが、**本 checklist の充足条件の追加**ではある（本文の pointer に加えて `promotion` を埋めることが要る）。
