...
- **watchdog（read-only・cron から回せる）**: `oe-undelivered`（子→親の報告が未受領のまま時間窓を越えたものを拾う）/ `oe-vitals`（統括セッションの拍動の鮮度と context% を見て、肥大接近とプロセス死を拾う）/ `oe-selfcheck`（外部の版に固定された前提が今も成り立つかを点検する）/ `oe-hookfire`（止める側のフックが直近の窓で発火したかを記録から読む）。いずれも engine の state（イベントログ / registry / session-state）は書き換えない。**ただし「何も書かない」ではない**: `oe-undelivered` と `oe-vitals` は二重通知を抑えるための自分用の記録（seen cache・観測の起点）を書くので、完全な read-only を前提にした環境では走らない。`oe-selfcheck` と `oe-hookfire` は何も書かない。判定の詳細と exit は各 verb の README を正本とする。
