...
4. **確定前に証跡をその場で残す**: 探索木＋SO 出力パス（`tmp/so-*`）＋採否を、確定と同一セッションで**確定前 artifact** に書く。置き場は手近なもの: `tmp/dj-N-tree.md` / ADR 草稿の「棄却案」節 / kickoff の DJ 節 / PR 本文 / review-log。**episode closure まで先送りしない**（「後で追記」は監査で 100% 不履行＝L4）。
...
<設計判断 DJ-N: ...>
├── 案A（初期） → 差分軸: ... → 採否: ❌ 理由 ...
├── 案C（ゼロベースで発見） → 差分軸: ... → 検証: ✅（so 出力: tmp/so-XXXX） → 採用
└── 未探索: <軸>（なぜ今探索しないか）
```
