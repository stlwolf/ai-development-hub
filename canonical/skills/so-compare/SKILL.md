...
- **claude は他レーンの2〜3倍かかる。** 一時的な揺らぎではなく、2件とも同じ傾向である。
- **所要時間はプロンプトの大きさでは決まらない。** 小さいほうのプロンプト（1.4KB）のほうが長くかかっている。効くのは要求される分析の深さであって、入力の量ではない。したがって事前に見積もりにくい。
...
- 1200秒は最大実測（777秒）に対して約54%の余裕である。リトライはレーンごとの基準から算出するので、claude のリトライは1800秒になる。

**重い SO を投げるときも、既定のままで claude レーンは返る。** タイムアウトを手で指定する必要はない。

実機で確認済みである。環境変数を一切設定せずレビュー級のプロンプトを投げ、`timeout_status=success` / 739秒 / 11748 bytes / リトライなしで返ることを確かめた。
