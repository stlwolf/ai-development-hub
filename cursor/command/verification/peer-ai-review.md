...
- ログファイルは `tmp/` 配下なので gitignore 対象。価値があればエピソードドキュメントに昇格させる
...
5. **Step 3 に戻る**: 改善した修正案とプロンプトで `so-compare.sh` を再実行
...
**ログ**: 最終結論セクション（合意状態・採用方針・残存リスク・根拠）を記入し、状態を「合意済み」または「上限到達・判断待ち」に更新する。
