...
AI駆動開発が当たり前になりつつある中で、「コーディングエージェントにどうタスクを振るか」「複数エージェントをどう協調させるか」というオーケストレーションの問題が出てくる。自前のオーケストレーションツールを検討するにあたり、既存のOSSツール21個を調査し、どんな概念・パターンで作られているかを整理した。

**注意**: 本記事は自分自身がオーケストレーションツールの設計を検討するために行った調査の整理であり、網羅的なベンチマークや推薦記事ではない。調査対象にはオーケストレーションツールだけでなく、関連するコーディングエージェント（Aider, SWE-agent等）や汎用エージェントフレームワークも含めている。21ツールを横断して見たときに浮かび上がる**設計パターンの全体像**を共有するものとして読んでほしい。

調査はAIサブエージェントを活用して**2026年2月下旬時点**の情報を収集・整理しており、各ツールの最新状況とは異なる場合がある。Stars数やステータスも調査時点のもの。

## 調査した21ツール

まず一覧。リポジトリへのリンクと調査時点のStars数を載せておく（2026年2月下旬時点）。
...
- **Claude Flow**（14.3k stars）も調査対象に含めていたが、コミュニティの検証でMCPツールの大部分がモック実装であることが指摘されたため、上記一覧からは除外した。Stars数と実装の実態は必ずしも一致しない
- **AutoGen**（54.7k stars）は2025年10月にメンテナンスモードに移行。Microsoft Agent Frameworkへ統合される方向
- **ControlFlow**はPrefect社CEOが開発したが2025年8月にアーカイブ。Marvin 3.0に吸収

## まず大きく2つに分かれる

21ツールを「何を自動化するか」で見ると、**ソフトウェア開発に特化したツール**と**用途を問わない汎用エージェントフレームワーク**に大きく分かれる。

### 開発特化型

ソフトウェア開発のワークフロー（コード生成・修正・テスト・レビュー・CI対応等）に焦点を当てたツール群。Docker隔離やgit worktree分離を備えるツールもあり、「コードを安全に書き換える」ことが設計の前提にある。
...
コーディングに限らず、任意のタスクのエージェントワークフローを構築するためのフレームワーク。ライブラリとしてアプリケーションに組み込む形が主流。
...
AI駆動開発の観点では、**開発特化型のワークフロー**を組みたいが、コンテキスト管理・ルーティング・フィードバックといった基盤設計は**汎用FW側の概念**を借りてくる、という関係がある。この記事では両方を同じ10の概念領域で分析した。その10領域は開発特化・汎用FWに共通する語彙になっている。

## 開発特化型の実装特性（2026年2月下旬時点）

開発特化型8ツールについて、ソフトウェア開発に関わる実装特性を比較する。
...
Library + Code-firstが主流で、Pythonが圧倒的。TypeScriptはMastra、BeeAIが対応。Google ADKが4言語（Python / Go / Java / TS）で最大のマルチ言語カバレッジを持つ。

## 10のドメイン概念領域

21ツールに繰り返し登場する概念を抽象化すると、10の領域に整理できた。これは開発特化型と汎用FW型の両方に共通する語彙になる。
...
レイヤー構造で見ると、下層がインフラ寄り、上層がセマンティック寄りになる。
...
OSSが厚く実装しているのは下層（定義・ランタイム・状態管理）と中層のフロー制御。上層の品質保証・人間介入はツールごとに設計判断が大きく分かれるか、そもそも薄い実装しかない領域だった。

AI駆動開発の文脈では、**04（Execution & Runtime）が特に重要な差別化ポイント**になる。Docker隔離、git worktree分離、ターミナル抽象化といった「コードを安全に触る」ためのインフラは、開発特化型ツールが独自に解決している領域で、汎用FWではカバーされない。

## 横断的な設計パターン

10領域に収まりきらない、複数領域にまたがるパターンをいくつか紹介する。
...
**ハイブリッド型（Council + Pipeline）** — 並列議論と逐次処理を組み合わせ。TAKTのTeam Leader Movement（タスク分解→並列実行→集約評価）がこのパターンに該当する。

**Swarm型（動的切り替え）** — Handoffによる動的な制御移譲。OpenAI Agents SDK、Google ADK等。柔軟だがLLMの判断に依存し、実行パスが予測しにくい。
...
ルーティングにおいて最も基本的な二項対立がこの2つ。

**Handoff**は制御の完全移譲。実行権がHandoff先に移り、元のエージェントは待機または終了する（OpenAI Agents SDK, Google ADK, AutoGen等）。**Agent-as-Tool**は制御を保持したまま他エージェントを「ツール」として呼び出し結果だけ受け取る（OpenAI SDK `agent.as_tool()`, AutoGen `AgentTool`等）。

「任せる」か「使う」かの違い。Handoffは会話履歴ごと移譲するため、移譲先が蓄積したコンテキストが元に戻ってきたとき膨張しやすい。Agent-as-Toolは呼び出し元が制御を保持するので予測しやすいが、呼び出し元がボトルネックになりうる。

### ルーティング判断の主体: LLM vs ルール

「誰がやるか」を決める判断を誰が行うかも、大きな設計分岐になる。

**LLM動的判断**はAgent Squad（Classifier）、Google ADK（`transfer_to_agent`ツール自動注入）等で採用。柔軟だが非決定的で、hallucinationリスクがある。**ルール静的判断**はMetaGPT（`cause_by`ベースの暗黙的ルーティング）やCrewAI（`@router`デコレータによる条件分岐）等で採用。実装方式は異なるが、共通して「LLMの判断に委ねず、事前定義されたルールで振り分ける」点が特徴。決定的で予測可能だが、事前にパターンを定義する必要がある。

実際には多くのツールがハイブリッドで、ルールで振れるものはルールで、それ以外をLLMに委ねるアプローチが多い。

### コンテキスト圧縮の4戦略

LLMのコンテキストウィンドウ制約に対処するための圧縮戦略も、21ツールを横断すると4つのパターンに分類できた。
...
開発特化型では、Aiderの「Repository Map」（tree-sitter + PageRankでコードベースの重要部分だけをトークン予算内で選択する）が最も洗練されたアプローチ。汎用FWではBeeAIが4種のメモリ戦略を設定で切り替え可能にしている。段階的開示はドキュメント配布のパターンとしてオーケストレーション設計に直接応用できる。

## まとめ

21ツールを調査して見えたことを3つにまとめる。

**開発特化型と汎用FWでは解くべき問題が違う。** 汎用FWはエージェント定義・フロー制御・状態管理といった抽象的な問題を解いている。開発特化型はそれに加えて、コード実行の安全な隔離（Docker/worktree）、コードベース理解（Repository Map, ACI）、CI/PRとの連携といった開発固有の問題を解いている。両方の知見を組み合わせることに価値がある。

**OSSが厚い層と薄い層がある。** 下層のインフラ（実行環境・状態管理・ツール接続）と中層のフロー制御はOSSが充実している。一方、上層の品質保証（ループ検出、多角的な検証）や、セッションをまたいだ知識管理はまだ発展途上で、ツールごとに独自のアプローチを模索している段階。自分で工夫する余地が最も大きいのもこの上層になる。

**プロトコルの標準化が進んでいる。** ツール接続のMCPは調査対象の多くが対応しており、エージェント間通信のA2Aも対応が広がりつつある。MCP（エージェントが何を使えるか）とA2A（エージェントが誰と話せるか）の二層構造が形成されつつある。

「このドメインにどんなツールがあって、どんな設計パターンが存在するか」の地図として参考になれば幸いである。
