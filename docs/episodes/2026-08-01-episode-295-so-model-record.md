...
- **SO 証跡**: 実装SO は `本文: N5: 実装SO で自分の設計の穴が2つ出た` に指摘と対応を転記。closure 自体の外部チェックは下記 Step4 を参照。
...
- 設計の前提を取り違えて退避方針を誤った。`本文: N5: 実装SO で自分の設計の穴が2つ出た`
- 握り潰し禁止の NK を入口にだけ適用し、後段の全経路に適用できていなかった。`本文: N5: 実装SO で自分の設計の穴が2つ出た`
- NK を守るつもりで書いた検証コードの中で、その NK が警告する早期終了 consumer を自分で踏みかけた。`本文: N5: 実装SO で自分の設計の穴が2つ出た`
- episode の記述が実装の変更に追随しておらず、Copilot に食い違いを指摘された。`本文: N5 後半: Copilot レビュー`
- cursor レーンが 300 秒で返らず、600 秒へ上げて 334 秒で返った。`本文: N5: 実装SO で自分の設計の穴が2つ出た`
...
- `canonical/` の sync が未実施。SKILL.md の変更を各ツールへ配る必要がある。**行き先: owner のマージ後の運用**（`./scripts/sync.sh`）。この委譲では sync しない。
