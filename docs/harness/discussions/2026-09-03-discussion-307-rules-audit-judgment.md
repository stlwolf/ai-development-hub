...
| #364 | 線引きの1行を2本へ | 12 | 350 | 4,430 | 350 | 4,430 |
| #365 | 探索クラスタ2本の縮約 | 12 | 300 | 3,728 | 300 | 3,728 |
| #366 | careful-operations を hook の正本へ | 12 | 275 | 3,714 | 275 | 3,714 |
| #367 | 本体と重なる指示の除去 | 12 | 272 | 3,694 | 272 | 3,694 |
| #368 | 関連リンクの行頭ラベル化 | 12 | 272 | 3,726 | 272 | 3,726 |
| **#369** | **見直し条件の付与（後）** | **12** | **320** | **3,934** | **284** | **3,726** |

**rule の frontmatter は読み込みの時点で剥がされ、本体にはファイル本文だけが載る**（統括が一時プロジェクトで実測。本文に置いた印は届き、frontmatter に置いた印は届かない）。だから #369 が足した 36行 208語の見直し条件は、file の語数を増やすが届く語数は増やさない。**#368 と #369 の届く語数は 3,726 で同値である。**

届く語数の前後は 4,477 から 3,726 で、**16.8% 減**。本数は 14 から 12。

**読み方の注意が3つある。**

- **増える PR が4本ある**（#360 #362 #364 #368）。削るより足す文のほうが長い場合、語数は増える。減らす向きの棚卸しでも各段が単調に減るわけではない。
- **最大の減りは1本（#365）で 702 語**。全体の減り 751 語の大半をこの1本が占める。**減らす効果は「経緯と例示を rule から出す」ことにほぼ集中していた。**
- **これは効果の測定ではない。** 遵守が上がったか、崩れが減ったかは測っていない。2026-04 の検証シナリオ（`docs/issues/38`）を書き換え後に再走すれば遵守率の前後は取れるが、本アークでは実施していない（follow-up）。

参考として shimo4228 は 5,789 語から 2,463 語（57.5%）だが、著者自身が効果の証明ではないと留保している。当方の 16.8% と比べる意味は薄い。**母集団が違う**（あちらは 20 本を 14 本へ整理、こちらは既に整理済みの 14 本が対象）。
