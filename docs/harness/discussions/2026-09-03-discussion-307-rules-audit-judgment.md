...
前: 14本 / 359行 / 4,477語（今日の `wc`）。後は plan の各 PR で測る。見込みは書かない（見込みを書くと目標になる）。参考として shimo4228 は 5,789 語から 2,463 語（57.5%）だが、著者自身が効果の証明ではないと留保している。外部対照として Piebald-AI の system prompt データ（版ごとの実トークン数）を次アークで開く候補にする。
