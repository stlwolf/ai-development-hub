...
**2つの数え方を分ける。** file の語数は `canonical/rules/*.md` をそのまま数えた値。届く語数は、常時ロードでモデルに載る部分だけを数えた値である。両者が分かれるのは #369 だけで、それより前は frontmatter が無いので同じ値になる。

| PR | 内容 | 本数 | file 行 | file 語 | 届く行 | 届く語 |
|---|---|---|---|---|---|---|
| — | master（前） | 14 | 359 | 4,477 | 359 | 4,477 |
| #360 | subagent-strategy の書き換え | 14 | 358 | 4,481 | 358 | 4,481 |
| #361 | skill-first-operations の退役 | 13 | 353 | 4,419 | 353 | 4,419 |
| #362 | implementation-principles の畳み込み | 12 | 350 | 4,425 | 350 | 4,425 |
| #363 | decision-pacing の縮約 | 12 | 347 | 4,367 | 347 | 4,367 |
| #364 | 線引きの1行を2本へ | 12 | 350 | 4,435 | 350 | 4,435 |
| #365 | 探索クラスタ2本の縮約 | 12 | 300 | 3,733 | 300 | 3,733 |
| #366 | careful-operations を hook の正本へ | 12 | 275 | 3,725 | 275 | 3,725 |
| #367 | 本体と重なる指示の除去 | 12 | 272 | 3,722 | 272 | 3,722 |
| #368 | 関連リンクの行頭ラベル化 | 12 | 272 | 3,754 | 272 | 3,754 |
| **#369** | **見直し条件の付与（後）** | **12** | **320** | **3,962** | **284** | **3,754** |

**この表は Copilot の指摘8件を反映した後の値である。** 指摘の修正で語が増えた段があるので、反映前より届く語数は 28 語多い。

**rule の frontmatter は読み込みの時点で剥がされ、本体にはファイル本文だけが載る**（統括が一時プロジェクトで実測。本文に置いた印は届き、frontmatter に置いた印は届かない）。だから #369 が足した 36行 208語の見直し条件は、file の語数を増やすが届く語数は増やさない。**#368 と #369 の届く語数は 3,754 で同値である。**

届く語数の前後は 4,477 から 3,754 で、**16.1% 減**。本数は 14 から 12。

**読み方の注意が3つある。**

- **増える PR が4本ある**（#360 #362 #364 #368）。削るより足す文のほうが長い場合、語数は増える。減らす向きの棚卸しでも各段が単調に減るわけではない。
- **最大の減りは1本（#365）で 702 語**。全体の減り 723 語の大半をこの1本が占める。**減らす効果は「経緯と例示を rule から出す」ことにほぼ集中していた。**
- **これは効果の測定ではない。** 遵守が上がったか、崩れが減ったかは測っていない。2026-04 の検証シナリオ（`docs/issues/38`）を書き換え後に再走すれば遵守率の前後は取れるが、本アークでは実施していない（follow-up）。

参考として shimo4228 は 5,789 語から 2,463 語（57.5%）だが、著者自身が効果の証明ではないと留保している。当方の 16.1% と比べる意味は薄い。**母集団が違う**（あちらは 20 本を 14 本へ整理、こちらは既に整理済みの 14 本が対象）。
...
- H1 の外の無題段落（自律運転・問題を述べている段では直さない・ターン終了前の点検・状態変更前の確認）は **Opus 5 には無い**。Fable 5.1 には既定 mode でも auto でもある。**差は permission mode ではなくモデル**で決まる。事前の期待「auto のみ」は外れた。4条件の実測は次のとおり（各条件 n=1・PRESENT / ABSENT はモデルの自己申告）。

| 項目 | fable 既定 | fable auto | opus 既定 | opus auto |
|---|---|---|---|---|
| (a) 自律運転・質問は作業を止める | PRESENT | PRESENT | ABSENT | ABSENT |
| (b) 問題を述べている段では直さず評価を返す | PRESENT | PRESENT | ABSENT | ABSENT |
| (c) ターン終了前に最後の段落を点検 | PRESENT | PRESENT | ABSENT | ABSENT |
| (d) 状態変更コマンドの前に証拠を確かめる | PRESENT | PRESENT | ABSENT | ABSENT |
| (e) 不可逆は確認・削除前に見る・忠実に報告 | PRESENT | PRESENT | PRESENT | PRESENT |
| (f) Writing for the user 節 | PRESENT | PRESENT | ABSENT | ABSENT |
| (g) Corrections 節 | ABSENT | ABSENT | PRESENT | PRESENT |
