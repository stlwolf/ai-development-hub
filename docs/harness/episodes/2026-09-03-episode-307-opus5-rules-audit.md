...
PR #360 から #369 まで 10 本を stacked で作った。マージはしていない。gate 4 は全 PR で回し、Copilot にも全 PR で依頼した。語数は届く分で 4,477 から 3,754、本数は 14 から 12（表は discussion §8。file の語数は frontmatter を含むので別列にしてある）。

**gate 4 が material な欠陥を挙げたのは 8 本**（#360 は2周・#361・#362・#364・#365・#366・#367・#369）。**そのうち6本はその PR の中で直した。** 残る2本は、#361 が配布側の掃除で別 issue へ回した分、#369 が実測で前提が崩れて直す必要が無くなった分である。8本のうち5本は同じ型だった。
...
## closure の外部チェックが挙げた欠陥（02:12〜）

heavy tier の外部チェック（`so-compare --with claude`・488 秒）を closure に当てたところ、**4観点のうち3観点で指摘が出た。** 自己評価は甘いという前提のとおりだった。主なものを挙げる。

- **Copilot の指摘8件を見ていなかった。** 全 PR で依頼したことは本文に書いたのに、結果を確認していなかった。#361 に1件、#364 に3件、#366 に1件、#367 に2件、#368 に1件。いずれも妥当な指摘で、1ラウンドで直して全件に返信した。内容は、指示対象が曖昧な文、定義語のように見える引用符、§2 への誤った参照、CATALOG と rule 本文の食い違い2件、そして **CLI Native の列挙に `cat` を入れながら同じ行でファイル操作を本体に委ねていた自己矛盾**である。最後のものは自分では気づけていなかった。
- **失敗の数え落としが2件。** 設計SO の1周目でレーンが落ちた件を closure の失敗の節に書いておらず、段階2 の並行実行だけを数えていた。外れた予測も3回と書いたが実際は6回だった。**外れた予測を少なく数えると、宣言する習慣の効き目を自分で低く見せることになる。**
- **揮発パスの扱いを事実と違う形で書いていた。** 「揮発パスの参照は本文に残っていない」と書いたが、実際には10箇所以上ある。うち2つは committed 側に写しがなく、そこにしか無い状態だった。
- **plan の状態が実行と合っていない。** チェックボックスが全行未チェックのまま、達成度だけ「達成」と書いていた。

**この4つはどれも、自分で読み返しても見つけられなかった種類である。** とくに Copilot の8件は、依頼して満足して結果を見ない、という形の抜けだった。

昇格の印: レビューを依頼したことと、レビューの結果を見たことは別の作業である。依頼した時点で完了扱いにすると、指摘が付いていても気づかない

## closure（gate 5・マージ前・2026-09-07）

tier は **heavy**。失敗と撤回があり（`本文: F-1 の実測 — 1回目（01:06〜01:10）` ほか）、品質ゲート目的で外部レビューを明示起動し（設計SO 2回・実装SO 12回・closure の外部チェック1回）、非自明な設計判断を棄却込みで持ち、昇格候補がある。
...
- **evidence anchor**: 本文には `.oe/` への参照が10箇所以上ある（作業層の report・claim・runtime capture）。**唯一の根拠になっていたものを2つ committed 側へ写した。** Opus 5 と Fable の4条件の PRESENT / ABSENT は discussion §11.1 の表へ、孤児 symlink の検査出力は `本文: 孤児 symlink の検査を実測した（探針を2回作り直した）` へ。残る `.oe/` 参照は、committed 側に要旨がある文書への provenance であって唯一の根拠ではない。**ただし `.oe/merge-packet-307.md` だけは例外で、owner のマージ判断の材料がそこにしかない。** これは #359 と同じ形の運用で、worktree を掃除する前に owner が読む前提である。掃除で消える性質は残課題に書いた。
...
- 計測器を自分で壊した3件。F-1 の対照レーンを補助関数の誤りで空にした（`本文: F-1 の実測 — 1回目（01:06〜01:10）`）。孤児 symlink の探針を正本の外に置き、faithful でない対照を取った（`本文: 孤児 symlink の検査を実測した（探針を2回作り直した）`）。dangling ref の検索でフィルタの前置を間違え、10件を誤って残存参照と読みかけた。**3件とも陽性対照か exit code の分離で気づけた。**
...
- レーンの脱落は2種類ある。**設計SO の1周目で codex と cursor が空のまま落ちた**のは、claim に「一次ファイルを開いてよい」と書いた自分の書き方が探索を誘発したためで（`本文: gate 2 — 1周目の結果（01:19〜01:23）: codex / cursor が timeout_empty`）、短縮した claim で回復した。**段階2 で codex が5本落ちた**のは SO を3本並行で回したためで（`本文: 段階2 の着地（02:10）— 10 本の PR と、そこで見つかった型`）、原因が別である。
- **Copilot の指摘8件に、closure を書く時点まで返信していなかった。** plan の共通の受け入れ基準は「Copilot レビュー依頼と gate 4 の結果を PR 本文に disclose する」を求めているのに、依頼したことだけを書いて結果を見ていなかった。closure の外部チェックがこれを突いた。8件はいずれも妥当で、1ラウンドで直して全件に返信した（`本文: closure の外部チェックが挙げた欠陥（02:12〜）`）。
...
- 測る前に期待値を宣言する習慣は、今回**外れた予測を6回**拾った。バイナリに環境変数は無いという予想（`本文: F-1 の計測器を先に確かめた（バイナリの文字列）`）、fable の長版が sonnet と同じ節集合だという予想（`本文: F-1 の実測 — 2回目（01:10〜01:13）と結論`）、Opus 5 の逐語引用が取れるという予想（`本文: Opus 5 採取の結果（03:38）と、切り分けが要る点`）、差が permission mode で決まるという予想（`本文: 切り分けの結果（03:41）: 期待が外れた。差は mode ではなくモデル`）、設計SO で3レーンとも黙るかもしれないという予想（`本文: gate 2 — 2周目と claude レーンの結果（01:27〜01:33）: 3 / 3 refuted・期待どおり論点4 と帰属を突かれた`）、孤児 symlink の探針が実物と同じ分類に落ちるという予想（`本文: 孤児 symlink の検査を実測した（探針を2回作り直した）`）。**外れたことが分かるのは、宣言してあるからである。**
...
| 残課題 | 行き先 |
|---|---|
| Cursor と Codex の runtime 採取 | 次アーク（skills 棚卸し）の最初の step。**次アークの issue はまだ無いので、#307 のコメントに引き継ぎとして残した** |
| rules 書き換えの効果測定（2026-04 の検証シナリオ再走） | #307 のコメントに残した。本アークでは追わない |
...
| 退役した rule の配布先の掃除 | マージ後に `./scripts/sync.sh --prune claude` を1回。`.oe/merge-packet-307.md` の §2 と PR #369 の本文に手順を書いた（packet は作業層なので、掃除で消えても PR 本文に残る） |
| shimo4228 の記事の 2026-08-27 更新の中身 | #307 のコメントに残した。Wayback が復旧したら見る。追わなくても実害は無い |
| F-1 を A/B の計測器として書き換えの前後を lean と長版の両方で測る | #307 のコメントに残した。効果測定と組で行う |
| F-1 のトークン数（見出し集合しか測っていない） | #307 のコメントに残した。優先度は低い |
| 発火実績を統括以外の役割で数える | #307 のコメントに残した。標本が統括1セッションに偏っている問題 |
| 反証レーンへ渡す claim の密度と、成果物をレーンが読める位置に置くこと（`promotion` の required） | 次アークの brief の固定節に入れる。#307 のコメントに残した |
| 棚卸しの基準線をモデル×版で宣言すること（`promotion` の required） | 次アークの brief の冒頭に入れる。#307 のコメントに残した |
