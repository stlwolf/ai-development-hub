...
## 段階2 PR-1: 宣言ファイルと読み取り専用の検査

owner が plan を承認した（2026-09-06）。裁定は `model` を宣言に入れない、個人層は hub も dotfiles も触らない、`enabledPlugins` は重ね合わせで入れる、`outputStyle` は #348 の判定まで入れず menu も使わない、drift も孤児 symlink も既定は報告だけ。PR-1 から PR-5 まで積み上げて作る。

### 実装SOが5周にわたって実在の欠陥を出し続けた

これが今回いちばん記録に値する。実装SO（`oe-review` 弱2レーン）を回すたびに、毎回それまで気付かなかった実在の欠陥が出た。しかも回を追って浅い所から深い所へ移っている。すべて実機で再現を確かめてから直した。

| 周 | 指摘 | 実在の確認 | 何が起きていたか |
|---|---|---|---|
| 1 | 葉のパスの取得に `paths(scalars)` を使っている | `{"a":false,"b":null,"c":1}` で `[["c"]]` しか返らない | 値が false と null の葉が落ち、重ね合わせの判定が空集合への全称になって無条件で緑 |
| 1 | 値の一致を `tojson` の文字列比較で書いている | `({a:1,b:2}|tojson) == ({b:2,a:1}|tojson)` が false | キーの順が違うだけで不一致になる。既存の検査は `jq -S` で正規化していたので退化させていた |
| 2 | 重ね合わせにスカラーの正本を与えると常に緑 | スカラーへの `paths` が `[]`、空集合への `all` が true | 手元の値が何であっても緑 |
| 2 | 項目を積む処理の失敗を握り潰している | コード上の `&&` の短絡 | 母集団が静かに縮み、残った項目だけで緑 |
| 3 | 解決先が消えた symlink を「未適用」と誤って報告 | `-e` はリンクを辿るので偽になり、`-L` の判定に届かない | 運用者に嘘の状態を伝える |
| 4 | `statusLine.command` が文字列でないと判定 jq が途中で終わる | 非文字列への `contains` が例外 | 先に出た hooks の判定だけで「宣言どおり」と報告 |
| 5 | 配列の添字を文字列前提で扱っている | jq の `paths` は添字を数値で返し、数値への `test` が例外 | 配列を含む正本で判定が丸ごと落ちる |

1周目の1件目は、自分がこの episode の「発見5」に書いた落とし穴そのものである。記録に残していたのに実装で踏んだ。知っていることと、書くときに思い出すことは別だった。

4周目で個別の型を1つずつ守るのをやめ、**判定できた件数が宣言の件数と合っているか**を突き合わせる形に変えた。同じ形の穴が別の場所に出うるので、穴を1つずつ塞ぐより母集団が縮んだこと自体を検出する方が効く。実際その後の5周目の欠陥も、この突合があれば緑にはならない。

この姿勢は同じリポジトリの `sync-bin.sh` が先に書いていたものと同じである。

> 母集団が壊れているときに「差分なし」を名乗らせない。0 件のまま回すと 0 回のループが緑を返し、検査が素通しになったことが緑と区別できなくなる。

昇格の印: 実装SOが5周連続で実在の欠陥を出した。「1周で足りる」という弱SOの既定の想定が、検査ロジックのように分岐が多く陽性対照を書きにくい対象では合わないのかもしれない。

昇格の印: 個別の型を守るより「母集団が縮んだことを検出する」方に投資すると、まだ知らない同型の穴もまとめて止まる。検査を書くときの一般則になりうる。

### レーンの返り方

codex レーンは通しで利用上限に当たり、一度も返っていない（`ERROR: You've hit your usage limit ... try again at 4:03 AM`）。cursor レーンは既定の 360 秒で一度時間切れになり、`SO_TIMEOUT=900` を与えてから安定して返るようになった。差分が7ファイルに増えたので既定では足りなかった。

統括から「SO は並行で回さない、レーンが空なら時間をずらしてそのレーンだけ再走する」という運用が来ている。今回は空を「返った」とは扱わず、時間を与えて再走した。

### テストの置き場

`projects/orchestration-engine/tests/` に置いた。`scripts/sync/sync-claude.sh` を対象にした既存のテストがそこにあるので、同じ場所に並べた。リポジトリ直下に `tests/` は無い。

## 段階2 PR-1 の締め: Copilot レビュー

Copilot から2件の行コメントとサマリーの指摘が来た。1件を採り、2件は理由をつけて断った。

採ったのは help の抽出の終端マーカーである。`set -uo pipefail` という固定文字列に依存していたので、フラグを足すと help が壊れる。緩い照合に変えた。同じ問題が後続の PR で足すスクリプトにも出るので、そちらも同じ形にした。

断ったのは `set -e` の追加である。この検査は差分をすべて数え上げてから結論を出す設計で、途中の非0で抜けると報告が尻切れになる。しかも呼び出し側から見ると差分ありと同じ終了コードになるため、走査が途中で終わったことと差分があったことを区別できなくなる。実装SOの4周目でまさにその経路が見つかっている。同じリポジトリの `apply-codex-notify-config.sh` も、警告して続ける設計のため同じ set 行である。ただし指摘の趣旨（失敗が見逃される）は正しいので、ガードが無かった `mktemp` は個別に確かめる形にした。

もう1件は「将来日付になっているメタデータ」という指摘だったが、これは誤りである。`date` の出力が 2026-09-06 で、記載した日付は本日である。

### Copilot への依頼の挙動が分かった

`gh pr edit <num> --add-reviewer @copilot` は exit 0 を返すが、`requested_reviewers` は空のままだった。bot slug を直接叩く経路でも同じだった。統括から、別の PR でも同じく空のままレビューだけが届いていると連絡があり、これが正常な挙動だと確定した。依頼は受理され、`requested_reviewers` には現れず、レビューだけが後から届く。memory の `reference_copilot_review_request_cli` にあった「PR 実依頼での実測は未」は、これで埋まった。

## 段階2 PR-2: 配布先の孤児 symlink を検出する

統括から追加スコープとして来た件（rule を退役させると壊れた symlink が残る）を、独立した PR にした。

### 実装SOがここでも4周続けて実在の欠陥を出した

PR-1 と同じことが起きた。しかも PR-1 とは違う種類の欠陥である。

| 周 | 指摘 | 何が起きていたか |
|---|---|---|
| 1 | bash 3.2 で空配列の再代入が落ちる | 上がる回数がパスの深さを超える相対リンクで、分類されないまま黙って落ちる。macOS の既定シェルがこの版 |
| 2 | リンク先が glob 展開される | 分割のために引用符を外していた。リンク先に星印が入るとディレクトリの中身に化けて分類が狂う |
| 2 | 配布先の走査の終了コードを見ていない | 読めないディレクトリがあっても黙って次へ進み、孤児が無かったのか見られなかったのかを区別できない |
| 3 | macOS の大文字小文字非区別 | 綴りだけ違うリンク先を正本配下と判定できず、孤児を見落として成功を返す |
| 3 | 名前に改行が入ると1件が2件に割れる | 行区切りで列挙していた |
| 3 | 正常時にも出力が増えていた | 「無ければ既存の出力を変えない」という自分で書いた受け入れ条件に違反していた |
| 4 | 解決できたのに字句判定へ落ちる | 正本配下の中間 symlink が外を指す形で誤分類。掃除の対象がずれる |
| 4 | 読み取り専用の検査が正本の隣にファイルを作る | 大文字小文字の判定にファイルを作って消していた。同名を切り詰めるし、割り込まれると残る |
| 4 | オプションの値欠けで終了コードが 1 | 文書には不正な引数は 2 と書いてあった |
| 4 | 辿れないディレクトリを「無い」と同一視 | 走査できなかったことが緑に化ける |

### テストが版を踏んでいなかった

3周目で気付いたことがある。テストを `/bin/bash`（3.2）で回しても、検査スクリプトは shebang 経由で PATH の bash 5 で動いていた。つまり「両系で緑」という主張が成り立っていなかった。いま走っているシェルでスクリプトを起動する形に直したところ、修正前のスクリプトに対して新しいテストが赤くなることを確認できた。

昇格の印: テストが対象を別の実行系で走らせていて、版差の主張が空だった。同じ形は「テストは通っているが対象を踏んでいない」という一般的な失敗で、他のテストにも当てはまりうる。

### 陽性対照の作り方を2回間違えた

最初に書いた回帰テストは、修正前のスクリプトに当てても緑だった。`..` の数をパスの深さから計算していたが、その計算では境界を踏まなかった。`/a/../../gone.md` という、途中で一度スタックが空になる最小の形に変えて、ようやく修正前で赤くなった。

期待した色が出たかではなく、期待した理由でその色が出たかまで見る、という教訓がそのまま当たった。修正前の版に当てて赤くならない回帰テストは、回帰テストではない。

昇格の印: 回帰テストは「修正前の版に当てて赤くなること」を確かめて初めて回帰テストになる。今回2回続けて、確かめずに緑を信じかけた。

## PR-3 の前に確かめる約束だった件: hooks は走行中のセッションに反映される

plan の「検証していない前提」に、hooks が走行中のセッションに反映されるかどうかを実装の前に確かめる、と書いていた。設計SOのレーンが「起動時に取り込まれ、途中の書き換えは反映されない」と述べ、その場では公式で確認していないと但し書きを付けていたためである。

公式ドキュメントに明確な記述があり、レーンの推測は誤りだった。
...
`hooks` は名指しで、再起動なしに走行中のセッションへ反映されると書かれている。設定ファイルの変更を監視していて、変更ごとに `ConfigChange` というフックが発火する仕組みもある（`user_settings` という matcher が `~/.claude/settings.json` の変更に対応する）。

一方、セッション開始時に一度だけ読まれるキーも明示されている。

- `model` — セッション中に変えるには `/model` を使う
- `effortLevel` と `modelSettings` — セッション中に変えるには `/effort` を使う
- `outputStyle` — システムプロンプトの一部なので、`/clear` か再起動の後に効く

つまり plan の DJ-7 に書いた「配布の後は既存セッションを再起動する、を運用の約束として書く」は言い過ぎだった。hub が配る `hooks` と `statusLine` は再起動なしで効く。再起動（あるいは `/clear`）が要るのは `outputStyle` で、これは #348 の判定を待って宣言に入れるかどうかを決めるキーである。PR-3 で plan の文言を直す。

検査の出力に書いている「走っているセッションの実際の値は公式に取得する手段がない」は、これとは別の話であり、そのままで正しい。どのキーがどのファイル由来かを外から取る手段が無い、という意味だからである。

昇格の印: 設計SOのレーンが但し書き付きで出した推測を、そのまま plan の前提に載せてしまっていた。但し書きは「確かめよ」の合図であって、確かめる前に前提として使う理由にはならない。

## PR-3 の準備: 現行の振る舞いを先に記録した

PR-3 は「適用を宣言から一般化する」ので、いちばん大事なのは現行と同じ結果を出すことである。それを示すには、先に現行の結果を記録しておく必要がある。

適用側の分岐を数え、8つの fixture を作った。settings.json が無い / hooks だけある / owner 独自の statusLine がある / すでに包まれている / JSON が壊れている / symlink である / 個人層のキーだけがある / ディレクトリになっている、の8つである。隔離した HOME で現行の `sync-claude.sh` を回し、結果の settings.json と終了コードとバックアップの個数を記録した。

記録して分かった、覚えておくべき現行の振る舞いが3つある。

**個人層のキーはすべて残る。** `model` `theme` `tui` `remoteControlAtStartup` `skipWorkflowUsageWarning` を置いた settings.json に対して sync を回しても、これらは一切変わらない。これが一般化で壊してはいけない性質である。

**JSON が壊れていると sync 全体が止まる。** hooks のマージが失敗して 1 を返し、`set -e` のもとで main が中断する。statusLine のマージまで届かない。settings.json はバックアップだけ取られて元のまま残る。statusLine 側には「壊れていたらそこだけ skip する」という手当てがあるのに、hooks 側には無いので、実際には hooks の側で止まる。

**バックアップは1回の sync で2つできる。** hooks と statusLine がそれぞれ独立に取るためである。PR-3 で書き込みを1回にまとめると、ここは1つに減る。これは意図した変化なので、等価性の比較から除く項目として先に決めておく。

昇格の印: 「現行と同じ結果を出す」を主張する前に、現行の結果を機械可読な形で記録しておく。後から比べようとすると、何が意図した変化で何が退化かを区別できなくなる。
