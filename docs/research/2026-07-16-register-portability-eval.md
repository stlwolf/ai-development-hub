...
### 段階2 追加材料（2026-07-17・形式仕様駆動との合流 + 他族 SO）

本ノート後、段階2 の追加材料が #263 に集約された（正本は issue 側・本ノートはポインタ）:

- **形式仕様駆動開発（formal master + 生成 NL + LLM 翻訳機）の外部議論と合流**し、追加検証要素 4.1〜4.5（優先順 4.1 > 4.2 > 4.5 > 4.3 > 4.4）+ 統括レビュー差分を #263 コメントに転記。統括レビュー差分の要点: 往復再構成(4.2)の採点は型付き claim 台帳(4.3)が先にあると機械照合になる（順序依存）/ 変異は分布内・単独事実に限る / token コスト台帳は token 単位。
- これら claim（同族 Claude 系のみで往復）を**他族5レーンで反証SO（弱・1周・4/5 返却・claude は timeout）**にかけた結果を #263 に転記（[他族SO反証コメント](https://github.com/stlwolf/ai-development-hub/issues/263#issuecomment-4994793535)）。**material に動いた点**: 「型付きエンベロープ vs 完全自由 prose」は偽の二分法で薄い統制 register（中間層）を選択肢に戻す要あり / 再測定は変異テストでなく**正解キー隔離を前提ゲート**にし評価器健全性と規範判断の2トラックに分離 / 単独事実限定の変異はグラフ整合の耐荷重事実を測れない。詳細は #263 コメント参照。
