...
## 計画/実行分離パターン（2026-02-20 追記）

FWアップグレード（2リポジトリ）の実践で有効性が確認されたパターン。

### 概要

「計画スレッド」と「実行スレッド」を分離し、GitHub Issue を「契約書」として橋渡しする。

```
計画スレッド: 調査 → SO → プラン → Issue化
  ↓ Issue（what: 本文 / how: コメント / learned: FB）
実行スレッド: Issue読み取り → 実行 → FB作成
  ↓ FB コメント
計画スレッド: FB確認 → 次タスクのプラン
```
...
| 層 | 内容 | 書き手 |
|---|---|---|
| **本文**（what） | タスクの定義、スコープ、受け入れ基準 | 計画スレッド |
| **コメント**（how） | コマンドレベルの実行手順（プラン転記） | 計画スレッド |
| **コメント**（learned） | 実行フィードバック（プラン vs 実際の差分） | 実行スレッド |

### 有効な条件

- ドメイン判断が少ない作業（アップグレード、インフラ構築等）
- 正解が収束しやすい（デファクトスタンダードがある）
- 検証が自動化されている（テストスイートが pass/fail で回答）

### 制約

- 実行スレッドのフィードバックを計画スレッドに戻す際、人間が明示的に指示する必要がある
- Cursor Plan ファイルはスレッド間で自動連携しない。Issue への転記が実用的
- 新機能実装など設計判断が多いタスクでは、Issue の「what」だけでは実装の選択肢を絞れない

### タスク適性マトリクス

| タスク種類 | 計画/実行分離 | SO の有効性 | 備考 |
|---|---|---|---|
| FWアップグレード | **高** | 事実確認に強い | 今回実証済み |
| インフラ構築 | **高** | 構成レビューに強い | デファクトがある |
| バグ修正（再現可能） | **中** | コードレビューに強い | facts.md + ロール設計（案B）が適合 |
| 新機能実装 | **中** | 設計レビューに使えるが合意しにくい | ADR + 受け入れ基準が追加で必要 |
| UX/ビジネス判断 | **低** | 限定的 | 人間の判断領域 |

### 実績

- 2026-02-20: FWアップグレード 2リポジトリで実施。両方 ~1.5h で完了、テスト全通過
- 同種タスクの2回目は SO ラウンドが減る傾向（1回目: 2ラウンド → 2回目: 1ラウンド）
