...
## セカンドオピニオン（SO）の効果と限界（2026-02-20 追記）

FWアップグレード（2リポジトリ）での実践から得た SO の運用知見。

### SO が高い価値を発揮する場面

| 場面 | 理由 |
|------|------|
| パッケージ互換性の事実確認 | レジストリ API で裏取り可能。人間の思い込みを修正できる |
| プランの構造レビュー（漏れ検出） | テストツール更新、設定ファイル修正など、プランに完全に欠落していた項目を発見 |
| PR の安全性レビュー | 後続タスクの発見、運用品質向上の指摘 |

### SO の価値が限定的な場面

| 場面 | 理由 |
|------|------|
| 自明な判断（自律実行範囲、見積もり等） | 自分で回答可能。SO のコストに見合わない |
| 同種タスクの2回目以降 | 初回のフィードバックが十分反映されていれば1ラウンドで合意する |

### SO 誤指摘の伝播リスク

**SO は安全ネットであって絶対的な品質保証ではない。**

- **ハルシネーション事例**: SO が存在しないパッケージバージョンを推奨。Codex / Claude 両方が同じ誤りを出した
- **伝播構造**: SO の指摘をプランに取り込み → 実行する流れは、SO の誤りを増幅する構造を内包している
- **検出メカニズム**: 今回は `composer update --dry-run` の失敗で検出（消極的な検出）。実行エージェントが盲目的に従っていたらブロックされていた
- **対策**: SO 指摘を取り込む前に、パッケージレジストリ API 等の一次データで検証するステップを標準化すべき

### SO の収束パターン

- 同種タスクの2回目は SO ラウンドが減る傾向がある（N=2、要再検証）
- プランが十分検証されていれば PR レビューは1ラウンドで完了する
- 3者合意ループ（peer-ai-review コマンド）により、合意プロセスが標準化された

### 一次データ検証の原則

「Evidence First」ルールがあっても、Web 検索結果を一次情報として扱ってしまう構造的問題がある。

**対策**: パッケージ互換性は Web 検索ではなくレジストリ JSON API を一次ソースにする。SO の指摘は「仮説」として扱い、一次データで検証してから取り込む。
