...
- `start` … `oe-delegate` を**使わない**。**登記の上では前任の子にしない**（素の pane として `claude` を起こし、`PARENT_TMUX_PANE` も渡さない）。**ただしペインの位置は別の話で、前任のペインを `-t` で指して割る**（`-t` を渡さない `split-window` はフォーカスしているペインを割るので、前任が自分のペインを割るつもりで撃っても別の window に出る。実地で踏んだ・2026-09-14。`oe-delegate` は同じ理由で `-t` を渡している）。**`TMUX_PANE` が空なら割る先が決まらないので撃たずに止める**（exit 2）。理由は、後継の自発的な報告の宛先がこれから停止する前任のペインに焼き込まれるからである（以前はもう1つ「後継自身が生きた委譲子に数えられて `take` が永久に通らない」という理由があったが、**子が交代を止めなくなったので効かない**・owner 裁定 2026-09-13）。起動と kickoff を分け、**ペインが現れなければ送らない**
...
- `retire` … 機械の検査をやり直して前任の申告と突き合わせ、食い違いを列挙する。閉じる直前に**そのペインがいまも前任のものか**を、**tmux server の世代**・pid・その pid のコマンド名で確かめる（session_id は引ければ突き合わせ、**引けないことでは止めない**）。server が再起動すると pane 番号も pid も作り直されて再利用されるので、**世代が違えば閉じない**。**閉じる相手の pane / session_id / pid は、目印の完全性を確かめた機械の節の中の値だけ**を使う（文書の散文から拾わない）。**申告の照合は「処分が付いた表の行」に対してだけ行う**（散文に番号を書いても処分にならない。PR は `#<番号>` の形で書く）。**停止の必須条件は1つで、申告の各項目に処分（`済んだ` / `引き継ぐ` / `取り消す`）が付いていること**。`--execute` は**検査をやり直してから**閉じるので、下見と実行のあいだに入った新着を拾う。**生きた委譲子と session_id は閉じない理由にしない**（下記）
...
**前提条件は緩く、確認は結果で取る（owner 裁定 2026-09-13・2件目）**: **交代を止めるのは「閉じる相手を取り違える経路」と「仕事が失われる経路」だけである。**

- **止めるもの**: 前任と自分が同じペインでないこと / board の席が自分を指し、引き継ぎ記録の前任と board の註が一致すること / **自己申告の各項目に処分が付いていること** / 閉じる直前の再検査（pid とコマンド名の突合・食い違いが見えたら止める）。
- **止めないもの**: 生きた委譲子の有無 / **前任の session_id が引けるかどうか** / transcript が使えるかどうか。いずれも**引けなかったことは記録して進む**（「引けた」と「引けなかった」は畳まない）。
- **なぜ緩めたか**: 「session_id が引けない＝停止が取り消せない」という前提が実態と合わない。**復帰の手段は別に在る**（`claude --resume` の一覧から選ぶ / 直後なら ID が分かっている / 分からなくても新しいペインから復帰できる）。実運用でこの故障は頻出していない。
- **確認は結果で取る**: 交代が成功したかは、前提を先に確かめるのではなく、**やったあとに観測できるもの**で見る。後継が cockpit に出ること（登記の root・`oe-tree`）・board の席が後継を指すこと（`take` の検算）・**通信が通ること**（後継と子のあいだで送って受領印が返る）。
- `prepare` に**人がセッション ID を渡す口は作らない**（owner が不要と判断）。
...
- **この穴は交代を止める理由にしない。** 止めるほうが害が大きいという裁定である（子を抱えた統括が context を使い切っても退けなくなる）。`take` と `retire` は引き受けた子を一覧で出す（**最新は `retire` の閉じる直前の数え直し**。**ただし数え直しに失敗した経路では一覧が出ない** — そのときは「誰を引き受けたかは確かめられていない」と断る）。**`retire` が穴の在り処を出すのは、`--execute` が停止まで通った経路だけである**（下見と、停止を拒んだ経路では出ない）。**登記のつけかえの案内は、そのうち閉じる直前の数え直しで0件と確かめられた経路だけ出さない**（数え直しができなかった `unknown` では出す）。`take` の側は「まだ打てない段階」なので未了として挙げるだけである。**「毎回明示する」とは書かない** — 実際に出る条件を確かめずにそう書いていたのを訂正した（2026-09-14・設計SO の指摘で条件を2つに分けた）。
...
**root が2本並ぶ件（2026-09-14 に書き直した）**: `oe-register root` は**前任の root の登記を失効させない**ので、交代の直後は `oe-tree` に `cockpit` の root が2本並ぶ。**ただし残り続ける歪みではない。** 登記への書き込みは末尾で GC（`lib/delegate-registry.sh` の `oe_reg_gc`）を呼び、GC は生存ペイン一覧に無い登記を消す。**後継が子を `oe-register link` でつけかえると、その書き込みが契機になって前任の root 登記も掃かれる**（1手で2つの症状が消える・実測 2026-09-13）。**子が0体の交代では、前任を停止したあとに登記へ触れる契機が無いので残る**（`take` も `oe-register root` を呼ぶが、それは前任がまだ生きている段階なので GC は前任の root を消せない。設計SO の指摘・2026-09-14）。 そのときは `oe-register root` を打ち直して契機を作れる（冪等）。**消えたかは木で見る**（GC は掃けないことがある）。以前は「この単位では直さない残っている歪み」と書いていたが、**手順1つで閉じることが分かったので訂正した**。lean の決定（`docs/decisions/2026-07-09-decision-238-239-succession-watchdog-lean-arch.md`）が「topology の歪みは段階1 の外」としている範囲にあたる。**後継が機械で状態を取るときにこの歪みを踏む**ので、見えた側で読み替える。
...
**呼び方の誤りは `broken` の帯（1）から外してある**（#309 残件・`oe-hookfire` と同じ扱い）。`-h` / `--help` は 0、unknown option・余分な位置引数は 2、必須の `jq` 不在も 2 である。**以前は help も unknown option も 1 を返しており、「呼び方を間違えた」が「`broken` を検出した」と同じ番号だった。**

**ただし全部の判定帯から切り離したわけではない。** `2` はいまも `indeterminate` の帯であり、**呼び方の誤りと前提コマンドの不在はそこへ相乗りしている。** 分離するかは #301 の決定待ちである。したがって **`2` を見て自動で再試行しないこと。** 前者2つは人が直すまで永久に再発するので、再試行は空振りし続ける。区別は stderr と `--json` にしか出ない。
...
- **呼び方の誤りは `broken` の帯（1）から外してある**（#309 残件）。`-h` / `--help` は 0、unknown option・余分な位置引数・`--days` の非整数は 2 である。**以前は help も unknown option も 1 を返しており、「呼び方を間違えた」が「`broken` を検出した」と同じ番号だった。**
- **ただし全部の判定帯から切り離したわけではない。** 上の行のとおり `2` はいまも `indeterminate` の帯であり、**呼び方の誤りはそこへ相乗りしている。** 承知のうえの妥協で、分離するかは #301（定期実行の配線）が exit を読むか JSON を読むかを決めてから判断する。
- **したがって `2` を見て自動で再試行しないこと。** 「呼び方が違う」は人が直すまで永久に再発するので、再試行は空振りし続ける。**どちらなのかは stderr と `--json` にしか出ない。**
