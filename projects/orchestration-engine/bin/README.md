...
**呼び方の誤りは判定の帯に入れない**（#309 残件・`oe-hookfire` と同じ扱い）。`-h` / `--help` は 0、unknown option・余分な位置引数は 2、必須の `jq` 不在も 2 である。**以前は help も unknown option も 1 を返しており、「呼び方を間違えた」が「`broken` を検出した」と同じ番号だった。** なお **`2` を見て自動で再試行しないこと。** `2` は「呼び方が違う」「前提コマンドが無い」と「検査が成立しなかった」を兼ねており、前者2つは人が直すまで永久に再発する。区別は stderr と `--json` にしか出ない。
...
- **呼び方の誤りは判定の帯に入れない**（#309 残件）。`-h` / `--help` は 0、unknown option・余分な位置引数・`--days` の非整数は 2 である。**以前は help も unknown option も 1 を返しており、「呼び方を間違えた」が「`broken` を検出した」と同じ番号だった。**
- **`2` を見て自動で再試行しないこと。** `2` は「呼び方が違う」と「検査が成立しなかった（`indeterminate`）」を兼ねている。前者は人が直すまで永久に再発するので、再試行は空振りし続ける。**どちらなのかは stderr と `--json` にしか出ない。** 判定を別の番号へ分離するかは、#301（定期実行の配線）が exit を読むか JSON を読むかを決めてから判断する。
