...
- **想定する脅威の範囲をはっきりさせてある。** 読むのは同じ作業ツリーで so-compare が自分で作った出力なので、守るのは「壊れた入力で黙って嘘の census を返さないこと」までである。**出力ディレクトリへ任意のファイルを書ける相手を想定した防御は、この道具では成立しない**（hard link は通常ファイルそのもので原理的に弾けず、検査と読み出しの間の差し替えも shell からは塞げない）。その状況では相手は同じ木のスクリプトや git hook も書き換えられるので、本 verb を固めても実質的な防御にならない。**弾けるものは弾き、弾けないものは verb 冒頭に書いて隠さない。**
