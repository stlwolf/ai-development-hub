...
> 1サイクル（QDD → so ゲート → 実装 → dogfood → Copilot レビュー3巡 → privacy 対応）を閉じる振り返り。

### 何が効いたか
- **QDD で実フローを抽出**したのが起点として効いた。「戻すより送る側が主眼」「割り込みは非問題」が早期に判明し、設計の重心が正しく定まった
- **so ゲートを計画(v1)と実装(v2)の2回**当てた。v1 で穴12件を事前に潰し、v2 で疎結合化の妥当性・回帰なしを確認。設計判断（疎結合化）を反証に通せた
- **dogfood が unit test で出ない edge を surfaced**: send-keys→Claude TUI ingestion の transient flakiness（Enter 吸収・stage 不達）は実セッションでしか出なかった
- **Copilot 再レビュー（3巡）が実害バグを捕捉**: 特に `oe_reg_gc` の「tmux 一時障害で registry 全削除」は静的レビューでしか気付けなかった重要な発見

### 設計判断の妥当性（事後評価）
- **疎結合化（delegate に report を内包しない）は正しかった**: kick パスは kickoff doc 読込込みで E2E PASS、戻しは汎用 `oe-send "$PARENT_TMUX_PANE"` で成立。「ワンラリーなら subagent の劣化版」という判断軸が dogfood で裏付いた
- **auto-Enter のコマンド内包（--no-enter）**も方向は正しいが、受け手 TUI 依存の transient 問題が残った（緩和A 止まり・根治は #114）

### プロセス上の学び（次に活かす）
- **so-compare `--prev` の UTF-8 footgun**: 前回回答のバイト切りで日本語が壊れ Codex が即死。`--prev` 除去 + `#`-free workspace + `SO_TIMEOUT` 増で解消
- **worktree × sync / filter-branch**: worktree から sync は footgun（記憶）。filter-branch は worktree でも range 指定で安全に実行できた
- **privacy 混入**: dogfood で別リポの識別子が episode に混入 → 前方スクラブだけでは履歴に残るため `filter-branch + --force-with-lease` で履歴ごと除去。**最初から伏せて書く運用**をメモリ化（[[no-cross-repo-private-info]]）
- **opt-in 戻しの手触り**: 子側が「自動で親に着信するはず」と旧モデルを期待 → opt-in のオンボーディング/明示が要る、という UX 知見

### follow-up（集約 ＋ 所感反映）
- **#114**（tmux/TUI 境界）: send-keys ingestion の**根治**（bracketed-paste 等）＋ `oe-capture` の pane-id 2系統（wez 整数 / tmux %N）統一。**オーナー判定: Enter 吸収は「バグ」扱いで緩和A止まりにせず根治を志向**（明示の `--no-enter` は OK、自動のはずが届かないのが不可）
- **論点E**: `oe-report` の alias 化 / 廃止で問題なし（**opt-in 戻しを採用＝自動付記の復活は不要**と確定）
- `--no-enter` の mid-session 長期観察・差分要因の分離
- **新規アイデア（本サイクル外・要 ideas 化）**: ①ビジュアライズド・オーケストレーション（インタラクティブ往復でなく、エージェント間のやり取りを可視化して人間が把握）②ターミナルマルチプレクサの自作可能性 ③4層 doc のコンテキスト引き渡しを自動注入でより暗黙的/フロー的に

### ユーザー所感（オーナー振り返り）

- **Issue（コンポーネント分割）は概ね期待どおりの動作・分割**だった。
- **tmux 経由のやり取りは想定より安定**（すぐ結果が出た点からも）。WezTerm CLI を作っていた頃の方が手間がかかった感覚で、tmux の安定性を踏まえると今の選択は妥当。一方で「**ターミナルマルチプレクサ自体を自作する**」可能性も少し感じた（将来の検討余地）。
- 横（関連の薄いスレッド）＋上下（親子）のラリーは、トークナイザ不具合はあったが**意図以上にラリーが回って**よく動いていた。今の形でも十分良い感触。
- **観察: 自前 HITL 運用ではピンポン過多が確認負荷になる**。決まったフローのオーケストレーションツール内で自動往復が入るなら良いが、HITL を自前で入れている今の運用では往復ごとに人間の確認が要り不安定さが出る。
  - → 案: やり取りの中で **フローを可視化した報告を別生成し人間が把握**できるようにする。「インタラクティブなセッション」ではなく **エージェント間の情報/やり取り自体を可視化する“ビジュアライズド・オーケストレーション”**（既存オーケストレーション構想の一コンポーネントとして以前から案あり）。
- **opt-in 戻しは「明示的にやると決めていれば全く OK」**。ただし **「自動(Enter)のはずが届かず手動 Enter が要る」のは処理として不正確＝バグなので直したい**（→ 論点E の自動付記“復活”は不要。直すべきは send-keys ingestion の Enter 吸収＝信頼性側）。
- **サイクルの重さはいつも通り**で違和感なし。今回 episode を書けた点も平常運転の範囲（最近サボりがちで、別リポでは記録を忘れることもあった）。
- **理想（締め）**: このフローの本体は **4層ドキュメント＝オーケストレーションのコンテキスト引き渡し**。今作っている自動注入（kickoff 注入）で、その**コンテキスト引き渡しをもっと暗黙的／フロー的に**処理できるのが理想（本サイクルとは独立の方向性）。
- **QDD 所感**: 初期はうまく伝わっていない箇所もあったが結局は伝わった。変な遠回りはなく概ね平常程度だが、**いくつか認識合わせに手間取った**（送る/戻すの重心、疎結合化の所など）。
