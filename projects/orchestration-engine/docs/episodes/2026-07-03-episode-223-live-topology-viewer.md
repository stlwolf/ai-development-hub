...
- **中身の中央寄せ（2 段で解決）**: 「左上すぎる」の実体は popup 枠でなく**パネル内の中身**の張り付き。(1) 中身をパネル中央に毎 tick 描画（TTY 時のみ・近似 wcwidth・非 TTY 無加工）を足したが「X 軸変わらず」の再報告。(2) 原因判明 = tmux popup 内で `tput cols` が実 pty を引けず 80×24 の fallback を返す（probe で実測: tput=80 / `stty size < /dev/tty`=187）→ サイズ取得を `stty size` 一次・tput fallback に切替えて解決。`-x C -y C`（popup 枠中央）は無害な明示指定として残置。教訓: 端末サイズ取得は popup 文脈で tput が嘘をつく — stty を一次にする。
