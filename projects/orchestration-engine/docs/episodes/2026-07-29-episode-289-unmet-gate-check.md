...
- **negative knowledge の収穫候補1件**（汚染経路の再導入パターン）→ **収穫済み。** owner が item 化を承認し、`projects/orchestration-engine/docs/knowledge/items/01KYSKQHDT86GZW87E8EDMSE46.md` として本 PR に収めた（`validate-knowledge` OK）。収穫時に**再発回数を実体で裏取りしたところ2点の補正が出た** — 1回目（設計段階）の一次記録は本 episode ではなく昇格先 discussion の「反証4パスで崩れた主張」節にあり、加えて digest の基準値を引数で受け取っていた件が同じ機構の3例目に当たる。したがって item は「狭く数えれば2回・機構で数えれば3回」と両方を明示している（上の「原則」項目の「2回」は狭い数え方である）。
