...
> **マージ後の訂正（PR #304・2026-08-03）**: 本 episode の学びを収穫する K 系単位が meta と突合し、2種を訂正した。**(1) 空返しの原因を「使用量上限による停止だったと判明した」と断定していた3箇所**（E6c 冒頭 / E6c 末尾 / E7 の「事実・失敗」）を、状況証拠の強さへ落とした。再実行が落とすのは「プロンプトがループする」「CLI が本質的にハングする」までで、原因の名指しは #303 の状況証拠にとどまる。**(2) E5 の SO 表の所要秒**を meta の実値へ合わせた（cursor 136 → 113 秒 / codex 425 → 450 秒。分量は元から一致していた）。訂正の経緯は `projects/orchestration-engine/docs/episodes/2026-08-03-episode-299-nk-harvest.md` の K6 節にある。
...
| レーン | 所要 | 分量 | 判定 |
|---|---|---|---|
| cursor | 113 秒 | 11.4KB | `partially-refuted` |
| codex | 450 秒 | 19.5KB | **`refuted`** |
...
## E6c. 遅れて返った3レーン目の指摘（新しい修正ラウンド）

E5 で claude レーンが 1200 秒 ×2 とも空だった件は、統括が同じ依頼（`-w` の3行だけ違う）を再実行して **888 秒・exit 0 で成功**した。**これで落ちるのは「プロンプトがループする」「CLI が本質的にハングする」という読みまでで、原因が使用量上限だと確かめられたわけではない**（meta 3本は原因を名指ししておらず、stderr も 0 バイトである）。使用量上限という名指しは **#303 に記録された状況証拠**（空だった時間帯が owner のリセット待ちと重なる・同じ実行の他レーンは成功している）にとどまる。**2レーンで締めた実装SO に、3レーン目が遅れて返った**形である。判定は `partially-refuted` で、方向と P0 は壊れていないが欠陥3件。

**そのレーンは sandbox で bash を1つも実行できておらず、全て静的読解でテスト green の申告も未検証だと自己開示していた。** 統括の指示も「まず各主張を自分で検証し、成り立つものだけ直せ」だった。**3件とも自分で検証し、3件とも成り立った。**

### (1) `screen-marker` が前提逆向きで、しかも `ok` に到達できない — 成立

**検証**: リポジトリ自身の fixture `tests/test_delegate_send.sh` の `scr_proc()` は `────── ws ──\n❯ \n──────\n  esc to interrupt` を返す。**目印は最終行＝入力欄より下**である。#144 の計画書 S1 も実機 capture を根拠に「`❯` 行と別行（**最下部** status 行）」と書いている。私が置いた「目印は入力欄より上に描かれる」は、**いま観測できる版の描画をそのまま前提にしたもの**で、設計時の版とは逆だった。

制御フローも確認した。`boxed>0` で先に `broken` を返すので、入力欄が下3行に在る（＝通常の配置）かぎり `in_window` の判定に到達せず、**`ok` は構造的に出せなかった**。検査として情報を持っていない。

**ここが一番重い。** 版に固定された前提を検出するための検査器が、**自分で版に固定された前提を置いていた。** しかも同じ tool から前のラウンドで「生涯累計で ok」を指摘され、そのときも「検査器が自分の検出対象と同じ壊れ方をしている」と書いたのに、**隣の検査で同じことを繰り返していた。** 1回目は直せたが、同じ型が他所にも在ることを自分では探しに行かなかった。

**直し方**: 位置の仮定を捨てた。見えた事実だけで判定する。陽性対照は「稼働中のペインが在ること」に置き、**目印にも版にも依存しない形として、短い間隔で2回 capture して内容が変わったペインを稼働中とみなす**。判定は「窓の中に目印を観測した→ok / どこかに在るが窓に入らない→broken / 稼働中なのにどこにも無い→broken / 稼働中が無い→indeterminate」。4通りをテスト [15] で固定した。

### (2) notify の guard が `new_keys` に掛かっていて空通知が飛ぶ — 成立

**検証**: stub の `wez` と `tmux` を置き、全行が `判定不可` になるログへ `--notify` を撃った。**`body=[] bodylen=0` の通知が実際に飛び、seen に 2 件焼かれた。** 既存ログの送信は nonce を持たないので全行が判定不可であり、**実ログへの初回 `--notify` がまさにこの経路**だった（統括の見積もりどおり空通知1発と seen へ331件）。

**直し方**: guard を `notify_lines` に掛け、seen へ焼くキーも**通知した行のものだけ**にした（`notify_keys` を対で持つ）。通知していない行を既読にしない。

**テストの穴も成立していた。** [9][10][23] は fixture に `delivery_receipt` が無く全て判定不可経路で、**本来通知したい「受領印なし」経路のテストが1つも無かった。** fixture へ nonce を持たせて経路を通し、新たに [24][25] で「判定不可だけなら通知も seen も無し」「受領印なしがあるときだけ通知し seen はその行だけ」を固定した。

### (3) 測定器が `unknown` を出力しない — 成立

**検証**: `unknown` 2件（うち1件は到達印あり）と `none` 1件、`suspected_miss` 1件を入れて実行したところ、**出力に `unknown` の行が一切現れなかった**。出力ループが `for sig in ("suspected_miss", "none")` と決め打ちだったためである。**#299 P0 で新設した値が丸ごと見えない＝「前後比較の基準」として成立していなかった。** 統括が追加させた指示に穴があった、という指摘もそのとおりである。

`key()` が正規化後 50 文字未満を `None` にして `continue` する経路も確認した。**短い kick は母集団にも未確認にも入らず、件数も出ずに静かに消えていた。** docstring の限界一覧にも無かった。

**直し方**: 出力をログに実在する値の列挙に変えた（`unknown` も出る）。短文で落ちた件数を signal ごとに数えて出力へ出した。docstring の限界一覧に両方を足した。あわせて入力先を env で差し替えられるようにした（**突合の方法は変えない**。保存済みログへ同じ方法を当てて「前後比較の前」を再現するための口で、これが無いとテストもできなかった）。

### この段の学び

**「同じ型が他所にも在るか」を自分から探しに行かなかった。** (1) は前のラウンドで指摘された型の再発である。指摘を受けた箇所を直して終わりにし、**同じ検査器の隣の関数に同じ型が残っていることを確かめなかった。** 指摘は1件ずつ来るが、型は横に広がる。

**そして「2レーンで締めた」判断そのものが早かった。** 0レーン不可という床は満たしていたが、**空返しの原因を確かめずに timeout と読んだ。** 時間をおいて再実行すれば返るものだった（原因そのものの名指しは #303 の状況証拠どまりである）。
...
- **次の消費者**: (0) 遅れて返った3レーン目の指摘（E6c）を反映済み。(1) この PR をレビューする owner。(2) transport の入れ替えを判断する単位（受領印と `scripts/measure-delivery-arrival.py` がその材料になる）。(3) `oe-selfcheck` の定期実行を配線する単位。(4) `oe-activity` の反転表示を是正する単位。
...
- **版依存を検出する検査器が、自分で版依存の前提を置いていた**（`本文: E6c. 遅れて返った3レーン目の指摘（新しい修正ラウンド）`）。前のラウンドで同じ型を指摘されていながら、隣の検査に残っていることを自分で探しに行かなかった。
- **空 body の通知が実際に飛ぶ状態だった**（同上）。実ログへの初回 `--notify` がその経路だった。
- **「前後比較の基準」として入れた測定器が、この PR で新設した値を出力していなかった**（同上）。
- **「2レーンで締めた」判断が早かった。** 空返しの原因を確かめずに timeout と読んだ（原因の名指しは #303 の状況証拠どまりで、確定していない）。
