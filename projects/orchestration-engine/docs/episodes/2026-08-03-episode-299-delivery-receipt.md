...
E5 で claude レーンが 1200 秒 ×2 とも空だったのは **使用量上限による停止**だったと判明した（統括が同一プロンプトを再実行し 888 秒 exit 0 で成功）。**2レーンで締めた実装SO に、3レーン目が遅れて返った**形である。判定は `partially-refuted` で、方向と P0 は壊れていないが欠陥3件。
...
**そして「2レーンで締めた」判断そのものが早かった。** 0レーン不可という床は満たしていたが、空返しの原因（使用量上限）を確かめずに timeout と読んだ。原因が分かれば待てば返るものだった。
...
- **「2レーンで締めた」判断が早かった。** 空返しの原因（使用量上限）を確かめずに timeout と読んだ。
