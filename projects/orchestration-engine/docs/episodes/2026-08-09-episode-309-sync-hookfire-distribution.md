...
- **`sync-bin.sh:39` の `head -n -1` が BSD で動かない件。** 実行して確認した（`--help` が `head: illegal line count -- -1` で exit 1）。本 PR の目的と無関係な既存不具合である。**行き先: 上の issue に隣接する欠陥として統括が記録する**（追加指示で明言）。本 PR では触らない、が「誰も追わない」という意味ではない
  - **【後日解決】** 後続単位 [`2026-08-09-episode-309-readme-enumeration.md`](./2026-08-09-episode-309-readme-enumeration.md) が `--list` を足した際、この関数が `--list` の案内経路そのものだったため判断を変えて修正した（`head -n -1` に加え、同じ行の `s/^# \?//` も BSD 非互換だった）。**統括による起票は不要。**
