...
promotion:
  ...
  - subject: "新しい入力源を env から取るとき、既存テストの env 制御範囲を確認する"
    verdict: not-required
    ref: "本文: 2026-08-28 Step 1（producer）完了"
  - subject: "SO は渡した量ではなく読ませる量で落ちる（既定の上限を仕事の形に合わせる）"
    verdict: required
    ref: "本文: 2026-08-28 gate 4 の再依頼（codex の欠けた1レーンを埋める）"
---
...
- **gate 4 を1レーンで確定しかけた。** codex の空返しを「codex の不調」と読み、cursor 1レーンで進めようとしていた。owner の指摘（「Codex は返してくることが多い」「渡し方の問題ではないか」）で切り分け直したところ、既定の上限が仕事の形に合っていないことが原因だった。**空返しは「意見が無い」ではなく「意見を取り損なった」だった**（`本文: 2026-08-28 gate 4 の再依頼（codex の欠けた1レーンを埋める）`）。
...
- **Anti**: SO の空返しをレーンの不調と読み、プロンプトを短くする方向で対処する。→ **Pattern**: 「読ませる量」を見て既定の上限を上げる（Step 5 で収穫）。

### 蒸留シグナル

- knowledge store: 3件収穫（`01M1272GA630SBY8ZAXBHC6JGH` 区切り文字 / `01M1272GA8CRXQKQWMF005NHCF` idiom 借用 / `01M141TB5YHB6VN62RA5TP0S3Q` SO は読ませる量で落ちる）。
...
| SO の渡し方（材料集めをレーンに読ませると既定の上限で足りない） | knowledge item `01M141TB5YHB6VN62RA5TP0S3Q` として収穫。`so-compare` スキルへの codify は owner 判断 |
| `so-compare` の codex レーンが継承した stdin でブロックする | **#352 に起票し PR #353 で修正済み**（`< /dev/null`・実測で 90 秒フル消費 → 3 秒） |
...
### 2026-08-28 gate 4 の再依頼（codex の欠けた1レーンを埋める）

owner の判断で実装SO を回し直した。**結果、再依頼は正しかった。**

条件を2つ変えた。`so-compare` を #352 の修正版（`< /dev/null`）に差し替え、`SO_TIMEOUT=600` にした（既定 240 の 2.5 倍）。前回以降に入った変更をコンテキストに追記し、doc よりコードを優先して読むよう指示した。

**codex は attempt1 で 308 秒かけて返した。** 既定の 240 秒では構造的に届かない仕事だったことが、これで確定した。

| レーン | 判定 | 内容 |
| --- | --- | --- |
| codex | **refuted** | material な欠陥3件（高2・中1） |
| cursor | survived | 前回の自分の指摘が直っていることを確認。残る事象（US 混入・display-message 単独失敗）は到達性が低く material でないと判断 |

**cursor が survived とした状態で、codex だけが3件見つけた。** 実装SO を2社にする理由（model 多様性で欠陥を捕まえる）が、そのまま出た形である。

| # | 指摘 | 私の評価と対応 |
| --- | --- | --- |
| 高 | `--all` が tmux 検証の前に `exit 0` する。ヘッダの「tmux 不在は exit 2」という契約に違反している | **成立。** ただし `--all` は sidecar だけを読むので tmux は本当に不要である。したがって直したのは**契約の側**で、前提がモードごとに違うことをヘッダ・usage・README に明記した |
| 高 | 置き場の探索不能が `nullglob` で 0 件に畳まれ、個々のファイルの permission / I-O error も `continue` で捨てられる。**全件読めなくても exit 0 で空表を返す** | **成立。しかも私のヘッダが自分で禁じている形だった**（「空表を 0 件として exit 0 で返さない・#322 DJ-3」）。置き場の可読性を明示検査し、jq の rc で読取失敗（rc=2）と中身の壊れ（rc=5 等）を分け、件数を stderr に出し、在るのに全件読めなければ exit 2 にした |
| 中 | `display_name` が配列や object でも producer が保存し、消費側の `@tsv` が rc=5 で落ちて **有効な ts / context_pct / pane ごと脱落する** | **成立。** producer 側で string に限定し、消費側でも型を強制して行を生かすようにした（既存の sidecar は直せないので消費側の防御が要る） |

修正の検証中に**自分でもう1つ間違えた**。読取失敗と中身の壊れを分けるために `PIPESTATUS[0]` を見ていたが、**command substitution はサブシェルなのでパイプの状態が親に伝わらない**。jq を単体で走らせて rc を取る形に直した。テストが2件落ちて気づいた（`在るのに全件読めない → exit 2` と `読めなかった件数を stderr に出す`）。

テストは [13] として6群17項目を追加し、bash 3.2.57 と 5.x の両方で 67/0。既存分も全件パス（producer 69 / registry 35 / home_unset 62 / vitals 77）。

昇格の印: SO の空返しは「意見が無い」ではなく「意見を取り損なった」である
