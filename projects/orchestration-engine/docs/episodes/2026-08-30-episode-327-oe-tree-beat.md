...
- **status 確定**: `stable`。達成度は **達成**（マージ後に確定・下記「達成の確定」節）。closure を書いた時点では **部分** としていた（実機確認がマージ後にしか取れないため）。
...
### 2026-09-01 達成の確定（マージ後・master への直コミット）

PR #354 がマージされた（master `af31a3b`）。primary tree を上げたところ、**cockpit が実際に叩く経路で拍動が出ることを確認できた**。

```text
$ command -v oe-tree      → /Users/eddy/bin/oe-tree（primary tree への symlink）
$ oe-tree --pick-list | cut -f2-
1.1   %0     alive  orch-12 ~<別リポ>
└─ 1.2   %16    alive  #99  Opus 5 (1M context) 65% ~<別リポ>
```

`~/bin/oe-tree` は primary tree の実ファイルへの symlink なので、**master へマージした時点で配備が済んでいる**（`./scripts/sync.sh --check` は4ターゲットすべて `up to date`）。検証したのは `--pick-list`、つまり `Ctrl+Space` → `v` の popup が読む面そのものである。したがって達成度を **達成** に確定した。

**この確定を、マージ前の closure に含められなかった。** owner から「episode はもう PR の前にしてほしかった」と指摘され、master への直コミットで是正することになった（owner の明示指示による例外運用）。

構造的な理由は、closure の必須項目（マージ前）が **owner 側のゲート（gate 6）の結果に依存する達成度を書こうとする**ことにある。実機確認が「マージ後にしか取れない」ものだと、closure 時点では必ず「部分」になり、確定はマージ後の追記に回る。回避するには次のどちらかだった。

- **配備の仕組みを先に確かめる。** 今回は symlink 配備なので「マージした瞬間に配備が済む」ことは事前に分かった。それを closure の時点で確認していれば、「マージ＝配備＝達成」と書けた（不要な「部分」を作らずに済んだ）。
- **達成度の対象を、この単位が制御できる範囲に限る。** 「実機で owner が目視」を達成度の条件に入れず、残課題として切り出す。

前単位は実機確認をせずに達成に近い報告をして未達だったので、今回は逆に慎重側へ倒した。**倒しすぎて、確定をマージ後へ押し出した**形である。

昇格の印: 達成度の条件に owner 側ゲートの結果を入れると、closure が必ず「部分」で終わる
