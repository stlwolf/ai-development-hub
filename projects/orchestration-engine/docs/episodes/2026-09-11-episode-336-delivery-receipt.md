...
### Copilot レビュー（到着・5 件すべて対応）

依頼から約 3 時間後に届いた。**5 件すべて妥当で、すべて直した**（1 ラウンド自律・全スレッドへ返信・再依頼はしない）。

1. **plist の PATH に `$HOME/bin` が無く `--notify` が黙って何も送らない。** launchd の job にはペインの tty が無いので OSC 777 の経路が使えず、通知は `wez notify` の fallback に落ちる。その `wez` は `sync-bin.sh` が `$HOME/bin` へ置くので、PATH から外れていると見つからない。**通知経路を作っておきながら、その経路が届かない配線をテンプレートに焼いていた。**
2. **brief の 1 行が「受領印が無い＝落ちた」という 2 値の判断を子に教えていた。** 本アークの設計はまさにその 2 値を避けるためにあるのに、**運用の指示文では 2 値に戻していた。** 実測で多数派（210 件中約 142 件）は「届いたが印を書けなかった」側なので、この指示に従うと多数派を誤診する。設計と指示文で言っていることが違った。
3. plan の S1-3 が状態を「4 値」と書き旧称 `silent` を残していた（実装は 6 状態・`unconfirmed` へ改名済み）。
4. plan の launchd 節が `OE_BOARD_FILE` / `OE_WATCH_REPORT_DIR` を渡すと書いていたが、`oe-confirm` はどちらも読まない。**そのとおりに配線すると report 検知が無効のままになる。**
5. `test_prompt_receipt` の 1 件が、コメントの筋書きを実際には踏んでいなかった（書き込めるディレクトリを渡したまま fallback 側だけを見ていた）。**通るが何も確かめていないテスト**で、gate 4 で cursor に「その種のテストは無いか」と明示的に聞いておきながら、自分で書いたものが該当した。

**2 と 5 は、私が同じアークの中で自分に課した基準を自分で破っていた例である。** 2 は「2 値にするな」という設計判断を運用文で崩し、5 は「通るだけのテストを書くな」とレビュアーに頼みながら自分で書いた。**設計の中で守った原則を、その周辺の成果物（指示文・テスト）へ持ち出せていない。** 1 と 4 も同型で、機能は作ったのに配線の記述が実装とずれていた。

あわせて §6 のリスク項が emit 位置の変更を反映しておらず事実として古かったので直した（Copilot の指摘ではなく、修正中に気づいた）。

昇格の印: 設計で確定した原則を、指示文・テスト・配線の記述へ持ち出せていない（4/5 件がこの型）
