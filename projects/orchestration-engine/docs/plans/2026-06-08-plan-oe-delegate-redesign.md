...
- **C（拡張 vs 新編）**: 新規 `oe-send` / `oe-list` 追加 + `oe-delegate` を spawn+send に refactor。`oe-report` は今回不触（論点E）。共有ロジックは責務分離:
...
- **E（戻し / oe-report 統合）**: 今回スコープ外。別 Issue 候補で保持
...
| 4 | `refactor(engine)` | `oe-delegate` を spawn+send 化（互換維持） |
...
- oe-report 使用手順の suffix 付記を **delegate 側で維持**（oe-send には載せない）
- `PARENT_TMUX_PANE` env + `/tmp/oe-parent-${child#%}` fallback の **両方を維持**（oe-report の親解決が依存）
- 位置引数の1行 task は従来どおり動作（`$1` セマンティクス維持）
- `--` デリミタ規約（#141）を崩さず `--kickoff`/`--label` を追加
