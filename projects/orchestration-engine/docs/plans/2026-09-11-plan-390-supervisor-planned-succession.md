...
### DJ-11: 生きた委譲子を抱えたまま交代する（**owner 裁定 2026-09-13 で書き換えた**）

**変更前の見出し**: 「生きた委譲子が0体であることは、停止の条件ではなく**席を動かす前の条件**にする」。

**owner 裁定（2026-09-13）で、この判断は不具合と裁定された。** owner の言葉は「子が生きている、むしろ生きてて交代することの方が多い。むしろそっちが正常な動作で、それで引き継げない今の状態がそもそも不具合。仕様じゃない」である。**行は消さない。** 変更前の本文はこの節の末尾（「変更前の本文（記録として残す）」）にそのまま置いてある。

#### 変更後の判断

- **子が生きたままの交代が正常系である。** 4つの verb はどれも子の数で交代を止めない。
- **責任の形**は「前任は子を引き渡す・後継は統括の仕事をまるごと引き受ける」である。前任が子を抱えて看取るのではない。
- `take` と `retire` は**子を数えて一覧で出すが、止めない**。後継が「誰を引き受けたか」を知るための表示である。
- `prepare` も子を理由に「まだ交代できません」と言わない。代わりに「後継へ子を引き渡す・後継は各子へ**新しい報告先を明示して伝えること**」を出す。
- **残る穴は1つだけで、出力と README に書く**（下記）。

#### 変更前の前提が実測で否定された

変更前の前提は「`PARENT_TMUX_PANE` は張り替えられないので、前任の pane を閉じると生きている子の戻し先が死ぬ」だった。**この前提は主経路に当たらない。**

- **統括の交代をまたいで同じ子が後継へ報告した実例がログにある。** `%157` が `2026-07-10T05:28:08Z` に `%144` へ、同日 `10:58:26Z` に `%158` へ送っている。`%144` と `%158` はどちらも当時の統括である。
- 理由は supervision が**リクエストとレスポンス**で回るからである。後継が子へ指示を送れば、子は**送ってきた相手に返す**。子は agent なので、環境変数ではなく「誰から来たか」を見る。
- `PARENT_TMUX_PANE` が効くのは、**誰にも聞かれずに子が自分から報告を出す経路だけ**である。supervision の主経路ではない。

**したがって変更前の判断は、狭い前提を主経路に一般化した誤りである。** 環境変数を読んだところまでは正しく、そこから「子は前任しか宛先を持たない」へ広げたところが誤りだった。

#### 残る穴（塞がる条件つきで残す）

自発的な報告は前任のペインを指したまま残る。**塞ぐには、後継が各子へ「報告の宛先は後継の pane である」ことを明示して伝える必要がある。**

**「一声かければ塞がる」と書いたのは不正確だった**（実装SO の指摘・2026-09-13 で書き直した）。`bin/oe-report` は親を「環境変数が先、無ければ `/tmp/oe-parent-<自 pane>` ファイル」の順で解決し、**`oe-delegate` はそのファイルを書き出さない**。差し替えの口は実質的に無いので、**宛先が変わるのは子が新しい pane を明示して送るときだけ**である。「後継が指示を送れば子は送ってきた相手に返す」は**子がそう振る舞うことに依存する**（子は agent なので送信元を見て返せるが、機構がそれを保証しているわけではない）。owner 裁定の骨（子は交代を止めない）は変わらないが、**塞がる条件は「挨拶」ではなく「新しい宛先を明示して伝えること」である。**

これを交代を止める理由にはしない（子を抱えた統括が context を使い切っても退けなくなるほうが害が大きい、という裁定である）。`retire` の出力は毎回この穴と塞ぎ方を明示する。

#### 設計SO の反証が効かなくなったこと（黙って落とさない）

変更前の本文は、設計SO の反証を受けて検査を `retire` から `take` の前へ移していた。反証の中身は「`retire` で止めると、席が動いてから停止までのあいだ前任と後継が併存し、DJ-11 が『子が居るあいだ前任を生かせ』と言うのでその併存が数時間に伸びる」である。

**この反証は前提が変わって効かない。** 併存が伸びる原因は「子が居るあいだ前任を生かす」ことだったが、**いまは子が居ても前任を生かさない**（`retire` が閉じる）。したがって併存は `take` から `retire` までの短い区間に戻る。**反証そのものが誤っていたのではなく、反証が前提にしていた判断が無くなった。** SO の指摘を落としたのではないことを明示するため、ここに残す。

同じ理由で、cursor が「DJ-11 の fail-closed という判断は lean の決定と整合する」と述べた点も、**fail-closed をやめたので当たらなくなった**。子の数え方の錨（下記）と「数えられなかったことを0に畳まない」という規律は**そのまま残る**（数えた結果で止めないだけで、数え方の正しさは変わらない）。

#### 後継を登記の子にしない判断は残る（理由は1つに減る）

変更前は理由が2つあった。(1) 後継自身が生きた委譲子に数えられて `take` が永久に通らない。(2) 後継の自発的な報告の宛先が、これから停止する前任のペインに焼き込まれる。**(1) は子が交代を止めなくなったので効かない。(2) だけで判断を維持する。**

#### 変更前の本文（記録として残す）

**ここから次の `###` 見出しまでは、2026-09-13 の裁定より前の本文である。** 現在の判断は上の「変更後の判断」であって、以下は**当時どう考えていたかの記録**として置いてある。読み替えずに残すのは、母集団を黙って縮めないためである。
...
| 指摘 | どのレーン | どう変えたか |
| --- | --- | --- |
| 生きた委譲子の検査を `retire` に置くと、席が動いてから停止までのあいだ前任と後継が無期限に併存する。検査は `take` の前に置くべき | codex | DJ-11 を書き直し、検査を `take` の前提へ移した。併存の期間が0 になる。**2026-09-13 追記: この反証は前提が変わって効かなくなった。** 併存が伸びる原因は「子が居るあいだ前任を生かす」ことだったが、owner 裁定で子が居ても前任を閉じるようになったので、併存は `take` から `retire` までの短い区間に戻る。**反証が誤っていたのではなく、反証が前提にしていた判断が無くなった** |
| 後継を `oe-delegate` で spawn すると後継自身が生きた委譲子になり、`take` が永久に通らない | claude | DJ-11 に「後継は `oe-delegate` で spawn しない」を足した。`prepare` が起動コマンドを表示する。**2026-09-13 追記: この反証も効かなくなった**（子が `take` を止めないので永久に通らない状態が起きない）。**判断そのものは維持する** — 理由は「後継の自発的な報告の宛先が、これから停止する前任のペインに焼き込まれる」のほうである |
| 生きた委譲子の数え方に錨が無い。`oe_reg_list` は自己スコープなので `take` では常に0体になり、fail-closed が fail-open へ反転する | claude | DJ-11 に錨（前任の pane を親とする entry を数える）を明記し、Step 1-4 で `lib/` へ切り出す。**2026-09-13 追記: この指摘は生きている。** 数えた結果で交代を止めなくなっても、**数え方が間違っていれば「誰を引き受けたか」が嘘になる**。錨と「数えられなかったことを0に畳まない」はそのまま残した |
...
- DJ-6 の圧縮の穴を「塞げない」と書いたこと自体は妥当だと3レーンとも認めた。ただし **DJ-11 の「塞げない」の書き方は狭すぎた**という指摘があり（塞げていないのは不便ではなく「前任が誰にも見られないまま子の戻し先で在り続ける」ことのほう）、これは DJ-11 の書き直しで解消した。**2026-09-13 追記: この指摘は当たっていた方向がさらに変わった。** 「前任が子の戻し先で在り続ける」ことが問題だったのではなく、**そもそも子の戻し先は前任に固定されていなかった**（応答の経路が主）。指摘は「狭すぎる」と言ったが、実際には**前提そのものが狭かった。**
- DJ-11 の fail-closed という判断そのものは lean の決定と整合しており、反証にはならないと cursor が明示した。**2026-09-13 追記: fail-closed をやめたので、この整合の指摘は当たらなくなった。** ただし「数えられなかったことを0に畳まない」という fail-closed の規律は、**表示の側に残した**（数えられなければ「確かめられていない」と書く）。
...
- [x] Step 1-4: 生きた委譲子の数え方を `lib/` に置く。**錨は前任の pane** で、`bin/oe-tree` の `children_of <pane>` に当たる経路を使う。`oe_reg_list` の自己スコープは使わない（DJ-11）
  - **この step は 2026-09-13 の裁定の影響を受けない。** 数えた結果で交代を止めないだけで、**数え方の正しさ（錨・自己スコープを使わない・数えられなかったことを0に畳まない）はそのまま要る**。後継が「誰を引き受けたか」を出すのに使う。
- [x] Step 1-5: `bin/oe-handoff` を新設し `prepare` を実装する。機械の節はまるごと上書きし、人の節は既存の内容を保つ。**生きた委譲子が残っていれば「後継へ引き渡す」と表示する**。後継を起こすコマンド（前任の pane を親としない素の pane で `claude` を起動する形）を表示する
  - **2026-09-13 に本文を書き換えた。** 変更前は「生きた委譲子が残っていれば**「まだ交代できない」**と表示する」。owner 裁定で子は交代を止めなくなったので、表示も「引き渡す」へ変えた（DJ-11 の「変更後の判断」）。**行は消していない。**
...
- [x] Step 2-6: `oe-handoff take` を実装する。順序は「確かめるものが先、書き換えるものが後」で、(1) 前提の検査（board が読める・前任の session_id が引ける・`prepare` の記録がある。**生きた委譲子は数えて出すが前提にしない**）(2) 見張りの状態を `oe-selfcheck --json` の行として読む（終了コードでは判定しない）と `oe-vitals` の登録・最終走査の確認 (3) `oe-register root --force --label cockpit` (4) board 張替（前任を「退任申告済み・停止待ち」として併記） (5) 検算 (6) イベント emit と読み直し (7) できたこと・できなかったことの表示
  ...
  - **2026-09-13 に本文を書き換えた。** 変更前の (1) は「前提の検査（**生きた委譲子0体**・board が読める・前任の session_id が引ける・`prepare` の記録がある）」だった。owner 裁定で子は席を止めなくなったので前提から外し、表示に回した（DJ-11 の「変更後の判断」）。**行は消していない。**
...
- [ ] Step 4-5: 受入。次の実際の交代で `prepare` → `take` → `retire` を1回通し、結果を episode へ追記する
  - → `projects/orchestration-engine/docs/plans/2026-09-12-plan-390-retire-and-docs.md` へ移設（本文は1文字も変えずに移した）
  - **結果（2026-09-13）**: 移設先で済んだ。本物の交代（14代目 `%84` → 15代目 `%104`）で通した。**この行にチェックを入れないのは、移設した step はすべて未チェックのまま「移設の記録」として残す、というこの節の規約に合わせるためである**（Step 4-1〜4-4 も同じ）。達成の記録は移設先にある。
- [ ] Step 4-6: 受入のとき、**`start` が送った kickoff が後継へ届いたことを受領印で確かめる**（DJ-10 の条件4）。`oe-confirm` の照合で、その送信の相関 ID に対する `prompt_received` が後継のペインから出ていることを見る。出ていなければ受入としない。**起動側で「入力を受け付けられる状態か」を機械で確かめられない以上、届いたことは後始末の側で見るしかない**（統括の申し送り・2026-09-12）
  - → `projects/orchestration-engine/docs/plans/2026-09-12-plan-390-retire-and-docs.md` へ移設（本文は1文字も変えずに移した）
  - **結果（2026-09-13）**: 移設先で済んだ。受領印は3つの述語をすべて満たした。チェックを入れない理由は Step 4-5 と同じである。
...
- **（解消済み・2026-09-13）生きた委譲子がいるあいだ交代そのものが始められない。** 変更前の本文は「DJ-11 の検査を `take` の前に置いたので、子が残っているかぎり席が動かない。『子を抱えたまま交代したい』場面では交代が止まる。その不便が繰り返し出るなら、lean の決定が defer した `@seat` mailbox の再開条件に当たる材料になる。**どのくらいの頻度で起きるかは未測定である。**」だった。
  - **owner が「子を抱えたまま交代するほうが多く、止まるのが不具合」と裁定したので、未測定のまま解消した。** 頻度の測定を待たずに通す形へ変えたので、**リスクとしては残らない**（代わりに残るのは自発報告の穴で、これは DJ-11 に書いた）。
  - **`@seat` mailbox の defer は解いていない。** 穴を塞ぐのは「後継が子へ一声かける」という運用で足りるという裁定であって、間接参照の口を作る判断ではない。
