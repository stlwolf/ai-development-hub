...
### Step 4-6（2026-09-13・統括のレビューを受けて）

**変更前**

> `oe-confirm` の照合で、その送信の相関 ID に対する `prompt_received` が後継のペインから出ていることを見る。

**変更後**

> 確かめる述語は次の3つをすべて満たすことである。(1) `oe-confirm --all --json` の照合で `received`（または `received-delayed`）と判定される。(2) 同じ `nonce` を持つ `prompt_received` が在る。(3) その `prompt_received` の `from.pane` が後継のペインである。

**なぜ書き換えたか。** 変更前の文面は「後継のペインから出ている」とだけ言い、**どの記録のどの欄を読むのかを言っていなかった。** そのため受領印の `to.pane` が空であることを見て「ペインの帰属が読めない」と誤読できた（統括のレビューで実際に起きた）。**条件を弱めたのではなく、読む欄を名指しして確かめられる形にした。** 満たすべきことは変更前と同じである。
...
- [ ] Step 4-5: 受入。次の実際の交代で `prepare` → `take` → `retire` を1回通し、結果を episode へ追記する
  ...
  - **予行として済んだ（2026-09-13・owner 指示）。** 枝の上で4つを1回通した。前任は使い捨てのペイン `%97`、board は稼働中の board の写し、登記とイベントは一時ディレクトリである。`take` = exit 0 / 下見1回目 = exit 1（申告が空なので閉じない）/ 申告を埋めたあとの下見 = exit 0 / `--execute` = exit 0（`%97` だけを閉じた）。結果は episode に追記した。
  - **本物の交代は残っている。** 予行は**統括の席を動かしていない**ので、この step は本物の交代で通すまで閉じない。予行と本番で条件が違う点（前任が新しいので `session_id` が引けない・後継が予行の足場を読める）も episode に書いた。
- [ ] Step 4-6: 受入のとき、**`start` が送った kickoff が後継へ届いたことを受領印で確かめる**（DJ-10 の条件4）。確かめる述語は次の3つをすべて満たすことである（**2026-09-13 に文面を書き換えた。変更前は「`oe-confirm` の照合で、その送信の相関 ID に対する `prompt_received` が後継のペインから出ていることを見る」。前後は「本文を書き換えた step（前後）」節にある**）。
  1. `oe-confirm --all --json` の照合で、その送信が `received`（または `received-delayed`）と判定される
  2. イベントログに、その送信の `message_sent.delivery_receipt.nonce` と同じ `nonce` を持つ `prompt_received` が在る
  3. その `prompt_received` の **`from.pane` が後継のペイン**である（`to.pane` は受領印では常に空で、**受け手のペインは `from` 側に載る**）
  出ていなければ受入としない。**起動側で「入力を受け付けられる状態か」を機械で確かめられない以上、届いたことは後始末の側で見るしかない**（統括の申し送り・2026-09-12）
  - **予行として済んだ（2026-09-13）。** 3つすべてを満たした。(1) `oe-confirm --all --json` が `received: 1`。(2)(3) nonce `01M2B66SKG8CY4TT48QKJVB975` を持つ `prompt_received` の `from.pane` が `%100`（後継）である。
  - **`to.pane` が空であることは #386 ではない。** 受領印は宛先を持たない記録なので `to.pane` は設計上空で、実物のログの `prompt_received` 799 件すべてが `from.pane` が埋まり `to.pane` が空である。#386 が起きる経路では**受領印そのものが書かれず**、受け手の診断に `no-tmux-pane` だけが残る。つまり #386 はこの述語の (2) が満たされない形で現れ、そのときは受入としない。
  - **本物の交代は残っている（Step 4-5 と同じ）。**
