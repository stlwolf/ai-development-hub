...
- [x] Step 3-2: 停止の必須条件を**2つ**入れる。前任の session_id が引き継ぎ記録に残っていること（`claude --resume` で開き直せる）・前任の申告の各項目に処分が付いていること（**2026-09-13 に本文を書き換えた。変更前は「必須条件を3つ入れる … 生きた委譲子が0体であること …」。前後は「本文を書き換えた step（前後）」節にある**）
