...
# **停止の直後がつけかえの打ち時である。** 前任が生きているうちは guard が拒むので、
# ここでしか打てない。間が空くほど %N が別のペインへ再利用される窓が広がる。
ckc "つけかえを打つよう案内する" "$out3" "いま打つこと: 引き受けた子の登記を自分の下へつけかえる"
# **素の名前では案内しない。** oe-register は PATH に無いので、そのまま打てる形で出す。
ckc "打つ verb を出す"           "$out3" "oe-register link"
ckc "解決できるパスで出す"       "$out3" "bin/oe-register link"
ckc "間を空けるなと言う"         "$out3" "間に別の作業を挟まないでください"
ckc "検算の見方を出す"           "$out3" "root の本数では見ません"
ck  "前任は閉じた"          "0" "$(grep -cxF -- '%10' "$ALIVE" | tr -d ' ')"
printf '%%10\n' >> "$ALIVE"
...
# **unknown を 0 に畳まない。** 数え直せていないのに案内を出さないと、
# 「確かめられていない」が「つけかえ不要」に化ける（設計SO の指摘・2026-09-14）。
ckc "unknown でもつけかえを案内する" "$out4" "いま打つこと: 引き受けた子の登記を自分の下へつけかえる"
ckc "誰を引き受けたか不明だと断る"   "$out4" "誰を引き受けたかは数え直せていません"
ckc "拾い方を示す"                   "$out4" "oe-tree で前任の下を拾ってから"
nck "居ない前提の文面にしない"       "$out4" "上に挙げた子の pane ごとに"
ck  "前任は閉じた"        "0" "$(grep -cxF -- '%10' "$ALIVE" | tr -d ' ')"
printf '%%10\n' >> "$ALIVE"
...
# 子0件の交代では、つけかえる相手が居ないので案内を出さない。
nck "子0件ならつけかえの案内を出さない" "$out12" "いま打つこと: 引き受けた子の登記"
